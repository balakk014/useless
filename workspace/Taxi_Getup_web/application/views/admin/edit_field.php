...
<div class="container_content fl clr">
	...
	<?php 
		if(isset($company_details[0]['field_labelname'])){
			$field_labelname = $company_details[0]['field_labelname'];
		}else{
			if(isset($postvalue['field_labelname'])){
				$field_labelname = $company_details['field_labelname'];
			}else{
				$field_labelname = "";
			}
		}
	...
	<div class="new_input_field">
              <input type="text" title="<?php echo __('enter_label_name'); ?>" name="field_labelname" id="field_labelname" value="<?php echo $field_labelname; ?>" maxlength="20" />
              <?php if(isset($errors) && array_key_exists('field_labelname',$errors)){ echo "<span class='error'>".ucfirst($errors['field_labelname'])."</span>";}?>
	</div>
	</td>   	
	</tr> 
	<?php 
		if(isset($company_details[0]['field_name'])){
			$field_name = $company_details[0]['field_name'];
		}else{
			if(isset($postvalue['field_name'])){
				$field_name = $company_details['field_name'];
			}else{
				$field_name = "";
			}
		}
	...
	<td>
	<div class="new_input_field">
               <input type="text" title="<?php echo __('enter_field_name'); ?>" name="field_name" id="field_name" value="<?php echo $field_name; ?>" maxlength="20" />
              <?php if(isset($errors) && array_key_exists('field_name',$errors)){ echo "<span class='error'>".ucfirst($errors['field_name'])."</span>"; } ?>
	</div>
	</td>   	
	</tr> 
	<?php 
		if(isset($company_details[0]['field_type'])){
			$field_type = $company_details[0]['field_type'];
		}else{
			if(isset($postvalue['field_type'])){
				$field_type = $company_details['field_type'];
			}else{
				$field_type = "";
			}
		}
	...
	<?php 
		if(isset($company_details[0]['field_value'])){
			$field_value = $company_details[0]['field_value'];
		}else{
			if(isset($postvalue['field_value'])){
				$field_value = $company_details['field_value'];
			}else{
				$field_value = "";
			}
		}
	...
	<td>
	<div class="new_input_field">
               <textarea name="field_value" id="field_value"  title="<?php echo __('enter_field_value'); ?>" rows="7" cols="35"><?php echo $field_value; ?></textarea>
	...
	<td colspan="" class="star">*<?php echo __('required_label'); ?></td>
			...
			<td>&nbsp;</td>
                        <td colspan="">
                            ...
                            <div class="button greenB">  <input type="submit" value="<?php echo __('submit' );?>" name="submit_editfield" title="<?php echo __('submit' );?>" /></div>
                            <div class="clr">&nbsp;</div>
                        </td>
                    </tr> 
...
$(document).ready(function(){
 ...
 var field_val = $("#field_labelname").val();
$("#field_labelname").focus().val("").val(field_val);
	toggle(8);
	$('#show_textarea').hide();
	show_value();
});
