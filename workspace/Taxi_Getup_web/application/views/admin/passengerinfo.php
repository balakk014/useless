       ...
       <div class="content_middle">    
          <div class="driverinfo_common">
		   ...
		   <?php if(isset($user_details[0]['referral_code'])) { echo $user_details[0]['referral_code']; } else { echo '-';} ?>
            ...
            <li><label><?php echo __('referral_amount'); ?></label>
	...
	<div class="title"><img src="<?php echo IMGPATH; ?>icons/dark/frames.png" alt="" class="titleIcon" /><h6><?php echo __('Ongoing Journey'); ?></h6>
		<form  action="<?php echo URL_BASE;?>manage/genpdf" method="post" name="drivermgmt" id="drivermgmt" >
		 <div class="title" align="right">
			 ...
			 <input type="text"  readonly title="<?php echo __('select_datetime'); ?>" id="ongoing_startdate" name="ongoing_startdate" value=""  />
			 <span id="ongoing_startdate_error" class="errors" style="display:none;"></span>
			 <?php echo __('enddate');?>
			 <input type="text"  readonly title="<?php echo __('select_datetime'); ?>" id="ongoing_enddate" name="ongoing_enddate" value=""  />
			 <span id="ongoing_enddate_error" class="errors" style="display:none;"></span>
			 <div class="button blackB"> 
			 <input type="hidden" name="type_export" id="type_export" value="">
			 <input type="button" name="btn_search_ongoing_trip" id="btn_search_ongoing_trip" value="GO" title="Go" >
			 
			 </div>				
		</div>
		</form>
	</div>
	<div id="ongoing_trip_logs">
		
			<?php if(count($passenger_logs_inprogress)>0) { ?>
			<table cellspacing="1" cellpadding="10" width="100%" align="center" class="sTable table-ongoing-trips responsive">
				...
				($i%2 == 1)?$class="eventr":$class="oddtr";
					...
					foreach($passenger_logs_inprogress as $values)
					{
						...
						<tr class="<?php echo $class; ?>">	
							...
							<td style="text-align:center;"><?php echo $values->no_passengers;?></td>
							<td><?php echo date('m/d/Y',strtotime($values->pickup_time));?></td>
							<td><?php echo date('h:i:s', strtotime($values->pickup_time));?></td>
							<td>
							<?php if($values->travel_status =='9' && $values->driver_reply =='A' || 
							...
							<?php 
							if($values->travel_status =='9' && $values->driver_reply =='A' || $values->travel_status =='3' && $values->driver_reply =='A')
							{
								?>
								<div class="button blackB">
