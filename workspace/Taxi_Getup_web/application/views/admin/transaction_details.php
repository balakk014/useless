...
$list = $split[3];
//echo "<pre>"; print_r($transaction_details[0]); exit;
?>
		   ...
		   <?php if($transaction_details[0]['payment_type'] !='3') { ?>
			...
			<?php /*   <tr>
			   ...
			   <li><label><?php echo __('amount'); ?></label>
                               <p><?php if(isset($transaction_details[0]['amt'])) { echo $company_currency.$transaction_details[0]['amt']; } else { echo '-'; } ?></p>
				   </li>

			   <li><label><?php echo __('paymentstatus_label'); ?></label>
                               <p><?php if(isset($transaction_details[0]['payment_type']) && $transaction_details[0]['payment_type'] != '0') { echo 'Paid'; } else { echo 'Not Paid'; } ?></p>
				   </li>
		</ul>
			...
			<?php } ?>
			  ...
			  <?php /* <tr>
			   ...
			   <li><label><?php echo __('Driver arrived time'); ?></label>
					<p><?php
						$arrived_time=isset($transaction_details[0]['arrived_time']) ? $transaction_details[0]['arrived_time']:"0000-00-00 00:00:00";
						if($arrived_time !="0000-00-00 00:00:00"){
							$arrived_time=Commonfunction::getDateTimeFormat($arrived_time,1);
						}else{
							$arrived_time="---";
						}
					?>
				   <?php echo $arrived_time; ?></p>
				   </li>
			    <li><label><?php echo __('pickup_time'); ?></label>
					<p><?php
						$pickup_time=isset($transaction_details[0]['actual_pickup_time'])?$transaction_details[0]['actual_pickup_time']:"0000-00-00 00:00:00";
						if($pickup_time !="0000-00-00 00:00:00"){
							$pickup_time=Commonfunction::getDateTimeFormat($pickup_time,1);
						}else{
							$pickup_time="---";
						}
					?>
			   ...
			   <li><label><?php echo __('drop_time'); ?></label>
				   <p><?php
						$drop_time=isset($transaction_details[0]['drop_time'])?$transaction_details[0]['drop_time']:"0000-00-00 00:00:00";
						if($drop_time !="0000-00-00 00:00:00"){
							$drop_time=Commonfunction::getDateTimeFormat($drop_time,1);
						}else{
							$drop_time="---";
						}
					?>
				...
				<li><label><?php echo __('total_time'); ?></label>
					   <p><?php
							if($pickup_time != "---" && $drop_time != "---") {
								echo $manage_transaction->dateDiff($drop_time,$pickup_time);
							} else {
								echo "---";
							}
                                                        ?></p>
			   ...
			   <li><label><?php echo __('Drop_Location'); ?></label>
				   ...
				   <?php echo ($drop_location != "") ? $drop_location : "--"; ?></p>
				   </li> 
			  ...
			  <?php /* <tr>
			   ...
			   <li><label><?php echo __('trip_minutes_fare').'('.$transaction_details[0]['trip_minutes'].__('minutes').')'; ?></label>
                               <p><?php if($transaction_details[0]['minutes_fare'] == '') { echo '-'; } else { echo $company_currency.round($transaction_details[0]['minutes_fare'],2); } ?></p>
				   </li>
			 ...
			 <?php /*   <tr>
			   ...
			   <li><label><?php echo __('waiting_time_hours'); ?></label>
                               <p><?php if(isset($transaction_details[0]['taxi_waiting_time']) && $transaction_details[0]['taxi_waiting_time'] != '' ) { echo $transaction_details[0]['taxi_waiting_time']; }else{ echo "--"; } ?></p>
				   </li>
			   ...
			   <?php } ?>
			     ...
			     <li><label><?php echo __('tax').'('.$transaction_details[0]['org_tax'].'%)'; ?></label>
				   ...
				   <?php
						$promo_calc ="-"; 
						$promo_code = (isset($transaction_details[0]['promocode']) && $transaction_details[0]['promocode'] !="")?$transaction_details[0]['promocode']:"";
						
						$percen =  (isset($transaction_details[0]['promocode']) && $transaction_details[0]['promocode'] !="")?"%":"";
						$promocode_discount =  (isset($transaction_details[0]['passenger_discount']) && $transaction_details[0]['passenger_discount']!="" && $transaction_details[0]['passenger_discount']!=0) ?$transaction_details[0]['passenger_discount']:"";

						if($promocode_discount !="")
						{
							$promo_calc = $transaction_details[0]['fare']*$transaction_details[0]['passenger_discount']/100;
							
							$promo_calc = $company_currency.round($promo_calc,2);
						}
					?>
				   
				   <li><label><?php echo __('Promocode'); ?></label>
                                 <p><?php echo (isset($transaction_details[0]['promocode']) && $transaction_details[0]['promocode'] !="")? $promo_code."(".$promocode_discount.$percen.")":"-";  ?></p>
				   </li>
				   
				   <li><label><?php echo __('Discount Amount'); ?></label>
                                 <p><?php echo $promo_calc; ?></p>
				   </li>
			   
			   <li><label><?php echo __('Sub Total'); ?></label>
				   <p><?php 
						/*if($passenger_discount > 0)
						{
			...
			<?php if($transaction_details[0]['driver_comments'] !='') { ?>
			   <li><label><?php echo __('reason'); ?></label>
                               <p><?php echo urldecode($transaction_details[0]['driver_comments']);  ?></p>
				   </li>
			   <?php } ?> 
			<?php if(isset($transaction_details[0]['comments']) && $transaction_details[0]['comments'] !='') { ?>
			   <li><label><?php echo __('reason'); ?></label>
                               <p><?php echo urldecode($transaction_details[0]['comments']);  ?></p>
				   </li>
			   <?php } ?> 
			   <?php if($transaction_details[0]['device_type'] == 1) { $dev=" (Android)"; } else if($transaction_details[0]['device_type'] == 2) { $dev=" (iOS)"; } else { $dev=""; } ?>
			<?php if($transaction_details[0]['passenger_app_version'] !='') { ?>
			  <!--  <li><label><?php echo __('passenger_app_version'); ?></label>
                               <p><?php echo $transaction_details[0]['passenger_app_version'].$dev;  ?></p>
					   </li> -->
			<?php } ?> 
			
			<?php if($transaction_details[0]['driver_app_version'] !='') { ?>
			  <!--  <li><label><?php echo __('driver_app_version'); ?></label>
                               <p><?php echo $transaction_details[0]['driver_app_version'].$dev;  ?></p>
					   </li> -->
			<?php } ?>

			<?php if($transaction_details[0]['booking_from'] !='') { ?>
			   <li><label><?php echo __('trip_type'); ?></label>
                               <p><?php echo ($transaction_details[0]['booking_from'] == 2) ? "Street Pickup Trip" : "Normal Trip";  ?></p>
					   </li>
			<?php } ?>
			...
			<tr>
			     <td colspan="2">
			       <table width="100%">
			       	 <tr>
			       		<td>
					       <?php $passenger_app_version = json_decode($transaction_details[0]['passenger_app_version'], true); ?>
					       <?php if(isset($passenger_app_version) && count($passenger_app_version) > 0) {?>
					       <table class="table table-bordered">
					         <tr><th colspan="2">Passenger Device Details</th></tr>
					         <tr>
					           <td>Device Type</td>
					           <td>
					           <?php if($passenger_app_version['device_type'] == 1){echo 'Android';} 
					           		elseif($passenger_app_version['device_type'] == 2){echo 'IOS';}else{ echo "-"; } ?>
					           </td>
					         </tr>  
					         <tr>
					           <td>Brand Name</td><td><?php echo ($passenger_app_version['brand_name'] != '') ? urldecode($passenger_app_version['brand_name']) :"-"; ?></td>
					         </tr>
					         <tr>  
					           <td>OS Versoin</td><td><?php echo ($passenger_app_version['os_version'] != '') ? urldecode($passenger_app_version['os_version']) :"-"; ?> </td>
					         </tr>
					         <tr>  
					           <td>App Version</td><td>
					           	<?php echo ($passenger_app_version['app_version'] != '') ? $passenger_app_version['app_version'] :"-"; ?>
					           </td>
					         </tr>
					       </table>
					       <?php } ?>
					     ...
					     <td>
					       <?php $driver_app_version = json_decode($transaction_details[0]['driver_app_version'], true); ?>
					       <?php if(isset($driver_app_version) && count($driver_app_version) > 0) {?>
					       <table class="table table-bordered">
					         <tr><th colspan="2">Driver Device Details</th></tr>
					         <tr>
					           <td>Device Type</td>
					           <td>
					           <?php if($driver_app_version['device_type'] == 1){echo 'Android';} 
					           		elseif($driver_app_version['device_type'] == 2){echo 'IOS';}else{ echo "-";} ?>
					           </td>
					         </tr>
					         <tr>  
					           <td>Brand Name</td><td>
					           <?php echo ($driver_app_version['brand_name'] != '') ? urldecode($driver_app_version['brand_name']) :"-"; ?></td>
					         </tr>
					         <tr>  
					           <td>OS Versoin</td><td><?php echo ($driver_app_version['os_version'] != '') ? urldecode($driver_app_version['os_version']) :"-"; ?> </td>
					         </tr>
					         <tr>  
					           <td>App Version</td><td><?php echo ($driver_app_version['app_version'] != '') ? $driver_app_version['app_version'] :"-"; ?></td>
					         </tr>
					       </table>
					       <?php } ?>
					     </td>
