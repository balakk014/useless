                    ...
                    <div class="taxi_scroll_one_top">
                        ...
                        <div class="col-md-4 col-md-4-inner">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group">
                                                                                    <input type="text" class="form-control" name="country_code" id="country_code"  maxlength="8" placeholder="<?php echo __('+1'); ?>" autocomplete="off" />			                               
                                ...
                                <div class="form-group col-lg-5_taxi_dispatcher">
                                    ...
                                    <div class="col-lg-12" style="margin-right: 0;">
                                        <input type="text" class="form-control" id="building_number" name="building_number" autocomplete="off" placeholder="<?php echo __('enter_building_number'); ?>" />
                                    </div>
                                    <div class="col-lg-12" style="margin-right: 0;">
                                        <input type="text" class="form-control" id="house_number" name="house_number" autocomplete="off" placeholder="<?php echo __('enter_house_number'); ?>" />
                                    </div>
                                    <div class="col-lg-12" style="margin-right: 0;">
                            ...
                            <div class="map_addbooking_outer">
                                <div id="map_addbooking"></div>
                                   ...
                                   <div class="col-lg-12" style="margin-top:20px;margin-right: 0;">
                                            <label><?php echo __('notes'); ?>:&nbsp;</label>
				   </div>
				<div>		
                                        <textarea rows="4" cols="40" id="driver_notes" name="driver_notes" autocomplete="off" placeholder="<?php echo __('enter_notes') ?>" /></textarea>
                                    </div>
                            ...
                            <?php /** booking details **/ ?>
									    ...
									    <div class="col-lg-12" style="margin-right: 0;">
										<input type="text" class="form-control" id="edit_building_number" name="edit_building_number" autocomplete="off" placeholder="<?php echo __('enter_building_number'); ?>" />
									    </div>
									    <div class="col-lg-12" style="margin-right: 0;">
										<input type="text" class="form-control" id="edit_house_number" name="edit_house_number" autocomplete="off" placeholder="<?php echo __('enter_house_number'); ?>" />
									    </div>
						...
						<div>		
						<textarea rows="4" cols="40" id="edit_driver_notes" name="edit_driver_notes" autocomplete="off" placeholder="<?php echo __('enter_notes'); ?>" /></textarea>
						</div>
                            </div>
                                                            </div>                                                                                
                                                            </div>
						...
						<tr>
							<td width="10%">
								<div class="new_input_field">
									...
									<input type="text" name="search_txt" maxlength="150" id="search_txt" value="" title="<?php echo __('search_manage_booking'); ?>" placeholder="<?php echo __('search_manage_booking'); ?>">
								</div>
							</td>
							<td width="10%">
								<div class="new_input_field">
							...
							<td width="5%" align="center">
								<div class="new_input_field">
									...
									<input type="button" name="submit_filter" id="submit_filter" title="Filter" onclick="all_booking_manage_list_search()">
									<input type="button" name="reset_date" id="reset_date" title="Reset">
								</div>
							</td>
						</tr>
					</tbody>
...
<script>
        ...
        $('#myModal').modal('show');
         ...
         //alert('as');
			...
			messages: {
				...
				country_code: {
					required: "The country code cannot be empty",
				},
				phone: {
					required: "The mobile number cannot be empty",
				},
				current_location: {
		...
		$(".edit_reset_btn").on('click',function(){
			...
			({ 			
				...
				success: function(response) 
				{
					...
					$('#edit_building_number').val(details.building_number);
					$('#edit_house_number').val(details.house_number);
					$('#edit_driver_notes').val(details.driver_notes);
       ...
       $("#dispatch").on('click',function(){
        ...
        $("#update_submit").on('click',function() {
			...
			if(edit_pickup_date_db != edit_pickup_date) {
				...
				if(hh < 1) {
					editValid = 0;
					$("#timeEditError").show().html("Later booking need minium 1 hour");
					return false;
				}
			}
			if(editValid) {
		...
		$("#create").on('click',function(){
			...
			if(hh < 1) {
				addValid = 0;
				$("#timeError").show().html("Later booking need minium 1 hour");
				return false;
			}
			if(addValid) {
		...
		<?php 	$b=1; 
			...
			for($i=0;$i<$a;$i++){ ?>
			    <?php echo $b; ?>: {
					...
					$drv_info='<span class="info-content">'.ucfirst(str_replace("'",' ',$all_company_map_list[$i]['name'])).'</span>';
					$drv_info.='</br>';
					$drv_info.='<span class="info-content">'.str_replace("'",' ',$driver_info).'</span>';
					$drv_info.='</br>';
	...
	function initMap(){
		...
		//map = new google.maps.Map(document.getElementById('map-canvas'), mainMap);
		var options = {
			componentRestrictions: {country: MAP_COUNTRY}
		};
		var search_map_location = new google.maps.places.Autocomplete(document.getElementById('search_map_location'), options);
		
		google.maps.event.addListener(search_map_location, "place_changed", function ()
		{
			/* var a = autocomplete.getPlace();
			GeocodeFromAddress();
			GotoLocation(a.geometry.location, 1) */
			var a = search_map_location.getPlace();
			GotoLocation1(a.geometry.location, 1)
		});
	}
	
	function GotoLocation1(a, b)
	{
		mainMap.setOptions({
			zoom: 90
		});
	...
	function all_booking_manage_list_search()
	{
		...
		var taxi_model = $("#select_taxi_model").val();
		var driver_status = $("#select_driver_status").val();
		if(new Date(filter_date) >= new Date(to_date))
        { 
            alert('To date must not Less than Start date..!');
            return false;  
        }
...
//                                         });
        ...
        if(new Date(filter_date) >= new Date(to_date))
        { 
            $('#to_date').css("border", "red solid 1px");
            return false;  
        }
		 $('.all_booking_manage_list').html("<div class='nodata'><p>Loading data.Please wait...</p></div>");
		//alert(status_cancel);
	...
	function setMarkers(locObj) {
		...
		//console.log(loc);
	        if (!locations[key] && loc.lat !== undefined && loc.lng !== undefined) {
				...
				}else if(loc.status=="S"){
					var status_icon="<?php echo BOOTSTRAP_IMGPATH.'/pass_icon.png'; ?>"; // PASSENGER MARKER
				}else{
