...
<link href="<?php echo URL_BASE; ?>public/logged_in/css/jRating.jquery.css" rel="stylesheet" type="text/css"/>
<script type="text/javascript" src="<?php echo URL_BASE; ?>public/logged_in/js/jRating.jquery.js"></script>
					...
					<ul>
						...
						<li>
							...
							<div class="view_trip_det">
								<a href="javascript:;" class="view_details pluse_icon" data-condition="1" data-id="<?php echo $rt; ?>-recent" title="View more detail"></a>
							</div>
						</li>
									...
									<script type="text/javascript">
										$(document).ready(function(){
											$(".basic<?php echo $r['passengers_log_id']; ?>").jRating({
												bigStarsPath : '<?php echo URL_BASE; ?>public/logged_in/images/detail_star.png', // path of the icon stars.png
												smallStarsPath : '<?php echo URL_BASE; ?>public/logged_in/images/small.png', // path of the icon small.png
												length : 5,
												rateMax : 5,
												step:true,
												showRateInfo: false,
					...
					<ul>
						...
						<li>
							<div class="trip_lst">
								...
								<?php } ?>
								<?php if($drop_location != "") { ?>
									<div class="passenger_drop_location"><p><?php echo $drop_location; ?> </p></div>
								<?php } ?>
                                  <p class="book_dat_time">
									  <?php echo $u["pickup_time"]; ?> <br>
									...
									?>
                                                                ...
                                                                <a class="cancel_trp" href="javascript:;" onclick="passengerCancel('<?php echo $u['passengers_log_id'] ?>','<?php echo $askCvv; ?>');"><?php echo __('cancel'); ?> trip</a>
							</div>
							<?php if($u["travel_status"] != 0 && $u["driver_reply"]!="")
							{
							?>
							<div class="view_trip_det">
								<a href="javascript:;" class="view_details pluse_icon" data-condition="1" data-id="<?php echo $up; ?>-upcomming" title="View more detail"></a>
							</div>
							<?php
							}
							?>
						...
						<?php if($u["driver_id"]!="" && $u["driver_id"] > 0) { ?>
						<?php
...
<div id="show_user_message">
    <div class="message_for_user_purpose">
        <h1>Please wait Do not refresh while cancelling the trip</h1>
        <img width="50" height="50" alt="loader" src="<?php echo URL_BASE;?>public/images/loading.gif"/>
    </div>
</div>
<div class="signin_form ask_cvv_popup" style="display: none;">
	...
	<div class="cvv_details">
		<ul>
			...
			<li>
				<div class="sub_butt">
					<input type="button" name="trip_cancel" id="trip_cancel_save" tabindex="2" value="<?php echo __('Submit'); ?>" title="<?php echo __('Submit'); ?>"/>
...
<script>
	$(document).ready(function(){
		...
		$('#trip_cancel_save').click(function(){
			...
			$("#card_cvv_error").html("");
			if(card_cvv == '') {
				$("#card_cvv_error").html("CVV should not be empty");
				return false;
			} else if(card_cvv.length < 3){
				$("#card_cvv_error").html("<?php echo __("cvv_length_error"); ?>");
				return false;
			} else {
				passengerCancel(tripId,0,card_cvv);
			}
		});
	});
function passengerCancel(tripId,askCVV,cvvVal)
{
	var cvvNo;
	if(askCVV == 2) {
		alert("You don't have credit card to cancel the trip");
		return false;
	} else {
		if(askCVV == 1){
       ...
       $.ajax({
           type: "POST",
           url: "<?php echo URL_BASE;?>find/cancelTrip",
           dataType: "json",
           beforeSend:function() {
           ...
           success: function (msg) {
               if (msg) {
				  ...
				  // }
                   ...
                   alert("You cannot cancel the trip !");
               }
           },
