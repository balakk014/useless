	...
	function initialize() {
	  ...
	  directionsDisplay_edit = new google.maps.DirectionsRenderer();
	  ...
	  map_edit = new google.maps.Map(document.getElementById('map_editbooking'), options);
	  directionsDisplay.setMap(map);
	  directionsDisplay_edit.setMap(map_edit);
		...
		google.maps.event.addListener(directionsDisplay, "routeindex_changed", function ()
		{
	  	...
	  	google.maps.event.addListener(directionsDisplay_edit, "directions_changed", function ()
		{
			updateStatus("Route changed");
			if (!working)
			{
				working = true;
				edit_GetElevation(directionsDisplay_edit.directions.routes[0])
			}
		});
		google.maps.event.addListener(directionsDisplay_edit, "routeindex_changed", function ()
		{
			updateStatus("Route index changed");
			edit_GetElevation(directionsDisplay_edit.directions.routes[directionsDisplay_edit.getRouteIndex()])
		});
		...
		google.maps.event.addListener(edit_autocomplete, 'place_changed', function() {
			...
			route1(origin_place_id, destination_place_id, travel_mode,directionsService, directionsDisplay_edit);
		...
		google.maps.event.addListener(edit_toAutocomplete, 'place_changed', function() {
			...
			route1(origin_place_id, destination_place_id, travel_mode,directionsService, directionsDisplay_edit);
			//console.log("lat->"+places.geometry.location.lat()+"long->"+places.geometry.location.lng());
