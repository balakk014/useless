...
Class Model_Mobileapi119 extends Model
{
   ...
   /** REGISTER FACEBOOK USERS **/
	...
	{		
		...
		$result=$query->execute()
				->as_array();
		$result[0]['name'] = urlencode($result[0]['name']);
		$result[0]['lastname'] = urlencode($result[0]['lastname']);	
		return $result;	
	}
...
'payment_type','order_time','amt','currency_code','payment_status','captured','admin_amount','company_amount','trans_packtype','payment_gateway_id'))
	...
	{	
		...
		$device_type = 2;
		$location_accuracy ='';
		if(isset($location_array['device_type']) && $location_array['device_type'] == 1){
			$device_type = $location_array['device_type'];
			$location_accuracy = $location_array['location_accuracy'];
		}

		//print_r($location_accuracy); 
		...
		if($driver_id!="")
		{
			...
			{					
				...
				if(count($trip_check)==0)
				{
					...
					$fieldname_array = array('driver_id','trip_id','active_record','location_accuracy','status','createdate','device_type');
					$values_array = array($driver_id,$trip_id,$location_record,$location_accuracy,$status,$current_time,$device_type);
					if($trip_id != 0)
					{
					...
					// We updated driver location waypoits one by one(lat,lon)
						...
						//Recursive lat,lon calculation Start
							...
							if($trip_id != 0)
							{
								...
								$updatequery = " UPDATE driver_location_history SET driver_id='$driver_id',status='$status',`active_record` = concat(`active_record`,'$location_record'),location_accuracy='$location_accuracy',distance='$total_distance'  where trip_id = '$trip_id'";
								$updateresult = Db::query(Database::UPDATE, $updatequery)
								->execute();
...
/* DATE_FORMAT(".COMPANY_MODEL_FARE.".night_timing_from,'%h:%i %p'),
 DATE_FORMAT(".COMPANY_MODEL_FARE.".night_timing_to,'%h:%i %p'), */
   ...
   {
   		$promo_type = "SELECT promocode_type FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code'";
   		$promo_type_result = Db::query(Database::SELECT, $promo_type)
					->execute()
					->as_array();
		$promocode_type = '';

		if(isset($promo_type_result[0]['promocode_type']) && !empty($promo_type_result[0]['promocode_type'])){
			$promocode_type = $promo_type_result[0]['promocode_type'];
		}
		if($promocode_type == 2){
			$promo_query = "SELECT promocode,promo_discount,promo_used,promo_limit FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code' and  FIND_IN_SET('$passenger_id',`passenger_id`)"; 
			
		}else{
			$promo_query = "SELECT promocode,promo_discount,promo_used,promo_limit FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code'";
		}
   ...
   public function checkpromocode($promo_code="",$passenger_id="",$company_id="")
   {
	 	...
	 	//$promo_query = "SELECT promocode,promo_discount,promo_used,start_date,expire_date,promo_limit FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code' and  `passenger_id` ='$passenger_id'  "; 
   		...
   		// Check promocode type for checking new user
   		$promo_type = "SELECT promocode_type FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code'";
   		$promo_type_result = Db::query(Database::SELECT, $promo_type)
					->execute()
					->as_array();
		...
		//echo $promocode_type;  exit;
		if($promocode_type == 1)
		{
		 	$promo_query = "SELECT promocode,promo_discount,promo_used,start_date,expire_date,promo_limit FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code'";
		 	//and  FIND_IN_SET('$passenger_id',`passenger_id`)  removed..!

			$promo_fetch = Db::query(Database::SELECT, $promo_query)
						->execute()
						->as_array();
		} else if($promocode_type == 2)
		{
			...
			$new_passenger_query = "SELECT COUNT(passengers_log_id) as pass_count,".PASSENGERS.".created_date FROM  ".PASSENGERS_LOG." JOIN  ".PASSENGERS." ON (  ".PASSENGERS_LOG.".`passengers_id` =  ".PASSENGERS.".`id` ) WHERE  `passengers_id` ='$passenger_id' and  travel_status='1' and driver_reply='A'";

			$new_passenger_count = Db::query(Database::SELECT, $new_passenger_query)
						->execute()
						->as_array();	

			$promo_query = "SELECT start_date FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code'";

			$promo_fetch = Db::query(Database::SELECT, $promo_query)
						->execute()
						->as_array();

			$passenger_create_date = date('Y-m-d', strtotime($new_passenger_count[0]['created_date']));
			$promo_start = date('Y-m-d', strtotime($promo_fetch[0]['start_date']));
			

			if($new_passenger_count[0]['pass_count']>0 && strtotime($passenger_create_date) < strtotime($promo_start)){
				return -1;
			}else{
				$promo_query = "SELECT promocode,promo_discount,promo_used,start_date,expire_date,promo_limit FROM  ".PASSENGER_PROMO." WHERE  promocode = '$promo_code'";
...
$sql = "SELECT trip_id,available_drivers,total_drivers,rejected_timeout_drivers,status FROM ".DRIVER_REQUEST_DETAILS." WHERE trip_id='$trip_id' and selected_driver='$driver_id' and status !='4' and createdate >= '$createdate' ORDER BY trip_id DESC LIMIT 0 , 1";
   ...
   public function previous_files_unlink($dir)
	...
	{
		$query = "select withdraw_request_id,request_id,withdraw_amount,request_date,request_status,brand_type from ".WITHDRAW_REQUEST." where company_id = '$company_id' and requester_id = '$driver_id' and type = '1' order by withdraw_request_id desc ";
		$result = Db::query(Database::SELECT, $query)
				->execute()
				->as_array();
		return $result;
	...
	public function insert_withdraw_request($company_id,$driver_id,$request_amount)
	{
		...
		if($result[0] > 0) {
			$update_query = "update ".PEOPLE." set trip_wallet = trip_wallet - '$request_amount' where id = '$driver_id' and user_type = 'D'";
			Db::query(Database::UPDATE, $update_query)->execute();
		} 
		return $result[0];
	}
...
*/		
	...
	public function cancel_withdraw_request($post)
	{
		$log_id=$post['log_id'];
		$date = commonfunction::getCurrentTimeStamp();
		$result = DB::insert(WITHDRAW_REQUEST_LOG, array('status','payment_mode','comments','withdraw_request_id','created_date'))->values(array(3,0,"Cancel by Driver",$log_id,$date))->execute();
		DB::update(WITHDRAW_REQUEST)->set(array('request_status' => 3))->where('withdraw_request_id', '=', $log_id)->execute();
	...
	public function search_withdraw_request_list($data)
	{
		//pe.id = $data['driver_id'] and
	
		
		$condition='';
		if(isset($data['driver_id']) && $data['driver_id'] !="" )
		{
			$condition .= " AND pe.id = '".$data['driver_id']."'"; 
		}
		
		if(isset($data['status'])  && $data['status'] !=3)
		{
			$condition .= " AND dw.status = '".$data['status']."'"; 
		}
		
		if(isset($data['from']) && $data['from']!=""){
			$condition .= " AND dw.date >='".urldecode($data['from'])."'";
		}
		if(isset($data['to']) && $data['to']!=""){
			$condition .= " AND dw.date <='".urldecode($data['to'])."'";
		}			
	
		$query = " SELECT dw.id as withdraw_request_id,t.fare as withdraw_amount,dw.status as request_status,dw.date as request_date,t.passengers_log_id as trip_id FROM `".PASSENGERS_LOG."` as pl left join `".TRANS."` as t ON pl.passengers_log_id=t.passengers_log_id left Join `".PEOPLE."` as pe ON pe.id=pl.driver_id left join `".DRIVER_WALLET_TRANS."` as dw ON pl.passengers_log_id=dw.trip_id  WHERE pl.travel_status = '1' and pl.driver_reply = 'A' and t.payment_type = 5 and t.fare >0 and dw.type = 2 $condition  order by pl.passengers_log_id desc";
