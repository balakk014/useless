...
class Model_Authorize extends Model 
{
	...
	public function changepassword($password,$userid)
	{
		...
		$rs=DB::update(PEOPLE)->set(array("password"=>$password,"org_password"=>$org_password))
				->where('id','=',$userid)
				->execute();
		if($rs>0){
	...
	public function edit_people($uid,$post)
	{
			...
			$result=DB::update(PEOPLE)->set(array('name'=>$post['firstname'],'address'=>$post['address'],'login_country'=>$post['country'],'login_state'=>$post['state'],'login_city'=>$post['city'],'lastname'=>$post['lastname'],'email'=>$post['email'],'country_code'=>$post['telephone_code'],'phone'=>$post['phone']))
				->where('id','=',$uid)
				->execute();	
	...
	public function edit_passenger($uid,$post)
	{
		...
		$result=DB::update(PASSENGERS)->set(array('name'=>$post['name'],'address'=>$post['address'],'email'=>$post['email'],'country_code'=>$country_code,'phone'=>$phone,'discount'=>$post['discount'],'passenger_cid'=>$post['company_id'],'twilio_number'=>$post['twilio_number']))
			->where('id','=',$uid)
			->execute();
	...
	{
		$query=DB::select('name','lastname','email','phone','address','login_country','login_state','login_city')->from(PEOPLE)->where('id','=',$userid)
					->limit(1)
					->execute()
					->as_array();
			return $query;
	...
	{
		$query=DB::select('name','email','phone','country_code','address','discount','passenger_cid','twilio_number')->from(PASSENGERS)->where('id','=',$userid)->limit(1)->execute()->as_array();
		return $query;
	}
	...
	**/
	...
	{
		$result = DB::select('id','name','lastname','email','phone','address','created_date','user_type','status')->from(PEOPLE)
					->where('user_type','!=','A')
					->where('status','!=','T')
					->order_by('created_date','desc')
					->limit($val)
   ...
   public function check_account_exist($userid){
	...
	/** passenger list **/
	public function all_passenger_list_history($offset, $val)
	{
		$query = "select id,phone,name,email,address,referral_code,wallet_amount,created_date,user_status from ". PASSENGERS . " where 1=1  order by created_date desc limit $offset,$val"; 
			$result = Db::query(Database::SELECT, $query)
						->execute()
						->as_array();
