...
class Controller_TaximobilityDashboard extends Controller_Siteadmin 
{
	...
	public function action_getallUsersCompanywise()
	{
		...
		if(isset($_POST['company'])) {
			...
			//$companies = $model_admin->get_company_details();//getting total company list
	...
	public function action_driver_status_details_search()	
	{
		...
		//$all_company_map_list = $manage_company->driver_status_details($driver_status);
	...
	public function action_dashboardTotalDetails()
	{
		...
		$view = View::factory('admin/dashboard/dashboard_total_details')
				->bind("dashboardData",$dashboardData)
				->bind("driver_commission",$driver_commission)
	...
	public function action_dashboardAssignUnassigenChart()
	{
		...
		$view = View::factory('admin/dashboard/assigned_unassigned_chart')
				->bind("company_name",$company_name)
				->bind("taxi_count",$taxi_count)
				->bind("assigned_taxi_count",$assigned_taxi_count)
				->bind("total_driver_count",$total_driver_count)
				->bind("assigned_driver_count",$assigned_driver_count)
				->bind("company_id",$company_id);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardCompanyWiseTripChart()
	{
		...
		$view = View::factory('admin/dashboard/company_wise_trip_chart')
				->bind("company_name",$company_name)
				->bind("trip_completed",$trip_completed)
				->bind("trip_inprogress",$trip_inprogress)
				->bind("trip_cancelled",$trip_cancelled);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardPaymentByCompanyChart()
	{
		...
		//print_r($payment_by_company);

		$view = View::factory('admin/dashboard/payment_by_company_chart')
					->bind("payment_by_company",$payment_by_company)
					->bind("company_id",$company_id);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardCompanyCountChart()
	{
		...
		$view = View::factory('admin/dashboard/company_count_chart')
					->bind("result",$company_count);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardTripReqTopCitiesChart()
	{
		...
		$view = View::factory('admin/dashboard/trip_request_top_cities')
					->bind("result",$result);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardcityByCountChart()
	{
		...
		$view = View::factory('admin/dashboard/city_by_count_chart')
					->bind("result",$result);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardcompanyRevenueChart()
	{
		...
		$view = View::factory('admin/dashboard/company_revenue_graph')
	...
	public function action_dashboardDriverRevenueChart()
	{
		...
		$view = View::factory('admin/dashboard/driver_revenue_graph')
					->bind('grpahdata',$grpahdata);
		echo $view; 
		exit;
	}
	...
	public function action_dashboardLiveDispatchMap()
	{
		...
		$view = View::factory('admin/dashboard/live_dispatch_map')
					->bind('all_company_map_list',$all_company_map_list);
		echo $view; exit;
	}
	
	
	// Driver Dashboard --- class ---- starts
	
	/**
	 * Driver Dashboard Total Details Chart
	 **/

	public function action_driver_dashboardTotalDetails()
	{
		$start_date = Commonfunction::ensureDatabaseFormat($this->request->post("startdate"),1);
		$end_date = Commonfunction::ensureDatabaseFormat($this->request->post("enddate"),2);
		$driver_id = $this->request->post("driver");
		//$brand_filter = $this->request->post("brand_filter");
		$brand_filter =1;
		$dashboard = Model::factory('dashboard');
		$dashboardData = $dashboard->driver_getDashboardData($driver_id,$start_date,$end_date);
		//print_r($dashboardData);
		$view = View::factory('admin/driver_dashboard/dashboard_total_details')
				->bind("dashboardData",$dashboardData)
				->bind("driver_commission",$driver_commission)
				->bind("driver_id",$driver_id)
				->bind("brand_filter",$brand_filter);
		echo $view; 
		exit;
	}
	
	/**
	 * Balance VS No balance Chart
	 **/

	public function action_driverbalanceChart()
	{
		$start_date = Commonfunction::ensureDatabaseFormat(date('Y-m-d 00:00:00'),1);
		$end_date = Commonfunction::ensureDatabaseFormat(date('Y-m-d 23:59:59'),2);
		//$company_id = $_SESSION['company_id'];
		$driver_id = $this->request->post("company");
		$dashboard = Model::factory('dashboard');
		$dashboardData = $dashboard->driver_getDriverBalance($driver_id,$start_date,$end_date);
		$balance_driver_count = (isset($dashboardData['balance_driver']) && $dashboardData['balance_driver']!="")?$dashboardData['balance_driver']:"";
		$no_balance_driver_count = (isset($dashboardData['no_balance_driver']) && $dashboardData['no_balance_driver']!="")?$dashboardData['no_balance_driver']:"";
		$driver_count =0;
		$driver_name="Drivers";
		
		$view = View::factory('admin/driver_dashboard/balance_driver_chart')
				->bind("balance_driver_count",$balance_driver_count)
				->bind("no_balance_driver_count",$no_balance_driver_count)
				->bind("driver_count",$driver_count)
				->bind("driver_name",$driver_name)
				->bind("driver_id",$driver_id);
		echo $view; 
		exit;
	}

	public function action_dashboardDriverTripChart()
	{
		$start_date = Commonfunction::ensureDatabaseFormat($this->request->post("startdate"),1);
		$end_date = Commonfunction::ensureDatabaseFormat($this->request->post("enddate"),2);
		$driver_id = $this->request->post("driver_id_two");

		$dashboard = Model::factory('dashboard');
		$result = $dashboard->getDriverTrip($driver_id,$start_date,$end_date);
		$driver_name = $trip_completed = $trip_inprogress = $trip_cancelled = "";
		if(count($result) > 0) {
			$company_name_array = $trip_completed_array = $trip_inprogress_array = $trip_cancelled_array = array();
			foreach($result as $trip) {
				$company_name_array[] = $trip["name"];
				$trip_completed_array[] = $trip["trip_completed"];
				$trip_inprogress_array[] = $trip["trip_inprogress"];
				$trip_cancelled_array[] = $trip["trip_cancelled"];
			}
			$driver_name = explode(",",addslashes(implode(",", array_map('ucfirst', $company_name_array))));
			$driver_name = "'" . implode("','", $driver_name) . "'";
			$trip_completed = implode(",",$trip_completed_array);
			$trip_inprogress = implode(",",$trip_inprogress_array);
			$trip_cancelled = implode(",",$trip_cancelled_array);
		}
			//print_r($driver_name); exit;
		$view = View::factory('admin/driver_dashboard/driver_trip_chart')
				->bind("driver_name",$driver_name)
				->bind("trip_completed",$trip_completed)
				->bind("trip_inprogress",$trip_inprogress)
				->bind("trip_cancelled",$trip_cancelled);
	...
	public function action_drivertopup()
	{
		$start_date = Commonfunction::ensureDatabaseFormat($this->request->post("startdate"),1);
		$end_date = Commonfunction::ensureDatabaseFormat($this->request->post("enddate"),2);
		$driver_id = $this->request->post("driver_id");
		...
		$dashboard = Model::factory('dashboard');
		$driver_topup = $dashboard->drivertopup($driver_id,$start_date,$end_date);
		$view = View::factory('admin/driver_dashboard/driver_topup')
					->bind("driver_topup",$driver_topup)
					->bind("driver_id",$driver_id);
		echo $view; 
		exit;
	}
	
	public function action_onlineOfflineChart()
	{
		$start_date = Commonfunction::ensureDatabaseFormat($this->request->post("startdate"),1);
		$end_date = Commonfunction::ensureDatabaseFormat($this->request->post("enddate"),2);
		$driver_id = $this->request->post("driver_id");
		$dashboard = Model::factory('dashboard');
		$result = $dashboard->getOnlineOfflineData($driver_id,$start_date,$end_date);
		$view = View::factory('admin/driver_dashboard/online_offline')->bind("result",$result);
		echo $view; 
		exit;
	}
	
	public function action_dashboardDriverDetailsChart()
	{
		$start_date = Commonfunction::ensureDatabaseFormat($this->request->post("startdate"),1);
		$end_date = Commonfunction::ensureDatabaseFormat($this->request->post("enddate"),2);
		$driver_id = $this->request->post("driver_id");
