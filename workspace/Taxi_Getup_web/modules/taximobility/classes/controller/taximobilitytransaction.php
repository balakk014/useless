...
class Controller_TaximobilityTransaction extends Controller_Siteadmin 
{
	...
	*/
	public function action_admintransaction()
	{
		...
		$passengerlist = $manage_transaction->gettransactionPassenger();
		$driverlist = $manage_transaction->getdriverdetails('','',array());
		$managerlist = $manage_transaction->getmanagerdetails('');
		/* $driver_manager_list = $manage_transaction->getdrivermanagerlist('','');
		$driverlist = $driver_manager_list['driver_list'];
		$managerlist = $driver_manager_list['manager_list']; */
		...
		$wallet_array[]=array ("pay_mod_id" => "5","pay_mod_name" => "Wallet","pay_mod_default" => 0);
		$gateway_details = array_merge($gateway_details,$wallet_array);
		...
		$grpahdata = $manage_transaction->getgraphvalues($list,'All','All','All','All','All',$startdate,$enddate,$offset, REC_PER_PAGE,'','','');

		$view = View::factory('admin/report/admintransaction')
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
				->bind('all_transaction_list',$all_transaction_list)
				->bind('count_transaction_list',$count_transaction_list)
	...
	*/
	public function action_admintransaction_list()
	{
		...
		$trip_id = isset($_REQUEST['trip_id'])?trim(Html::chars($_REQUEST['trip_id'])):'';
		$manage_transaction = Model::factory('transaction');
		$common_model = Model::factory('commonmodel');
		...
		if(($manager_id !="") && ($manager_id !="All") || ($usertype == 'M')) {
			...
			if(count($manager_details) > 0) {
				foreach($manager_details as $m) {
					$manager_details_array[] = array("login_country" => $m['login_country'],"login_state" => $m['login_state'],"login_city" => $m['login_city'],"company_id" => $m['company_id']);
				}
			}
		}
		...
		$passengerlist = $manage_transaction->gettransactionPassenger($company);
		...
		/* $driver_manager_list = $manage_transaction->getdrivermanagerlist($company,$manager_id);
		$driverlist = $driver_manager_list['driver_list'];
		$managerlist = $driver_manager_list['manager_list']; */
		...
		$grpahdata = $manage_transaction->getgraphvalues($list,$company,$manager_id,$taxiid,$driver_id,$passengerid,$startdate,$enddate,$offset, REC_PER_PAGE,$transaction_id,$payment_type,$payment_mode,'',$taxi_ids,$driver_ids,$trip_id);
		...
		if(isset($_SESSION['download_set'])) {
			...
			//echo "<pre>"; echo count($all_transaction_list); exit;
			...
			if(count($all_data) > 0) {
				foreach($all_data as $key => $value) {
					...
					if($value['actual_pickup_time'] != '0000-00-00 00:00:00') {
			...
			//$export_table_header = array(__('cctransaction_id'),__('payment_type'),__('trip_id'), __('passenger_name'),__('driver_name'),__('journey_date'),__('passenger_email'),__('Current_Location'),__('Drop_Location'), __('companyname'), __('admin_commision'), __('company_commision'), __('waiting_time_with_format'), __('distance_km'), __('trip_total_fare'), __('nightfare'), __('eveningfare'));

			$export_table_header = array(__('trip_id'), __('passenger_name'),__('driver_name'),__('journey_date'),__('passenger_email'),__('Current_Location'),__('Drop_Location'), __('companyname'));
			if($list != 'rejected') {
			...
			//$export_table_field_select = array('transaction_id','payment_type','passengers_log_id','passenger_name','driver_name','journey_date','passenger_email','current_location','drop_location','company_name','admin_amount','company_amount','waiting_time','distance','fare','nightfare','eveningfare');
			$export_table_field_select = array('passengers_log_id','passenger_name','driver_name','journey_date','passenger_email','current_location','drop_location','company_name');
			if($list != 'rejected') {
		...
		$view = View::factory('admin/report/admintransaction')
				...
				->bind('get_allcompany',$get_allcompany)
				->bind('grpahdata',$grpahdata)
				->bind('graphStartDate',$graphStartDate)
				->bind('graphEndDate',$graphEndDate)
				->bind('gateway_details',$gateway_details)
				->bind('grpahstartdate',$startdate)
				->bind('id',$id);
	...
	*/
	public function action_adminrejcancel()
	{
		...
		$view = View::factory('admin/report/adminrejcancel')
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
	...
	public function action_companytransaction()
	{
		...
		$passengerlist = $manage_transaction->gettransactionPassenger($company_id);
		...
		$wallet_array[]=array ("pay_mod_id" => "5","pay_mod_name" => "Wallet","pay_mod_default" => 0);
		$gateway_details = array_merge($gateway_details,$wallet_array);
		//echo '<pre>';print_r($gateway_details);exit;
		//pagination loads here
		$page_no= isset($_GET['page'])?$_GET['page']:0;
		...
		$grpahdata = $manage_transaction->getgraphvalues($list,$company_id,'All','All','All','All',$startdate,$enddate,$offset, REC_PER_PAGE,'','','', $passenger_ids,array(),'','');
		//****pagination ends here***//

		//send data to view file 
		
		$view = View::factory('admin/report/companytransaction')
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
	...
	{	
		...
		$grpahdata = $manage_transaction->getgraphvalues($list,$company_id,$manager_id,$taxiid,$driver_id,$passengerid,$startdate,$enddate,$offset, REC_PER_PAGE,$transaction_id,$payment_type,'',$passenger_ids,$taxi_ids,$driver_ids);
		$graphStartDate = Commonfunction::getDateTimeFormat($startdate,1);
		$graphEndDate = Commonfunction::getDateTimeFormat($enddate,1);
		//****pagination ends here***//
		...
		if(isset($_SESSION['download_set'])) {
			...
			if(count($all_data) > 0) {
				foreach($all_data as $key => $value) {
					...
					unset($value['eveningfare']);
					$all_data[$key]['eveningfare'] = $eveningfare;
					$fare = ($value['fare'] != 0) ? $ccur.' '.round($value['fare'],2) : '-';
					unset($value['fare']);
					$all_data[$key]['fare'] = $fare;
				}
			}
	...
	public function action_managertransaction()
	{
		...
		$grpahdata = $manage_transaction->getgraphvalues($list,$company_id,$user_createdby,'All','All','All',$startdate,$enddate,$offset, REC_PER_PAGE,'','');
		//****pagination ends here***//

		//send data to view file 
		
		$view = View::factory('admin/report/managertransaction')
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
	...
	public function action_managertransaction_list()
	{
		...
		$grpahdata = $manage_transaction->getgraphvalues($list,$company_id,$user_createdby,$taxiid,$driver_id,$passengerid,$startdate,$enddate,$offset, REC_PER_PAGE,$transaction_id,$payment_type,'','',$taxi_ids,$driver_ids);
		$graphStartDate = Commonfunction::getDateTimeFormat($startdate,1);
		$graphEndDate = Commonfunction::getDateTimeFormat($enddate,1);
		//send data to view file 
	...
	public function action_gettaxilist()
	{
		...
		$gettaxi_details = $manage_transaction->gettaxidetails($company_id,'',array());
	...
	public function action_getdriverlist()
	{
		...
		$manage_transaction = Model::factory('transaction');
		$getdriver_details = $manage_transaction->getdriverdetails($company_id,'',array());
	...
	public function action_getpassengerlist()
	{
		...
		$manage_transaction = Model::factory('transaction');
		$getpassenger_details = $manage_transaction->gettransactionPassenger($company_id);
 ...
 <table border="0" cellpadding="0" cellspacing="0">';
		...
		if($list != 'rejected') { 
			...
			//$xls_output .= "<td>".__('package_type')."</td>";
		} 
			...
			//$xls_output .= "<td>".__('passenger_email')."</td>";
			$xls_output .= '<td class="head_border">'.__('Current_Location').'</td>';
			$xls_output .= '<td class="head_border">'.__('Drop_Location').'</td>';
			//$xls_output .= "<td>".__('companyname')."</td>";
			//$xls_output .= "<td>".__('No_Passengers')."</td>";
			if($list != 'rejected') { 
			//$xls_output .= "<td>".__('admin_commision')."</td>";
			//$xls_output .= "<td>".__('company_commision')."</td>";
			} 
		...
		if($list != 'rejected' && $list != 'cancelled' ) { 
			...
			//$xls_output .= '<td class="head_border">'.__('equivalent_to_usd').CURRENCY_FORMAT.'</td>';
			//$xls_output .= "<td>".__('nightfare')."</td>";
			/*$xls_output .= "<th>".__('rating_points')."</th>";
			$xls_output .= "<th>".__('comments')."</th>";*/
		} 
		elseif($list == 'cancelled') { 
			$xls_output .= '<td class="head_border">'.__('fare').'</td>';						
			$xls_output .= '<td class="head_border">'.__('tax').'</td>';
			...
			//$xls_output .= '<td class="head_border">'.__('reason').'</td>';
		} 
		$xls_output .= "</tr>"; 
		$file = 'Export';
		...
		foreach($list1 as $result)
		{
			if($list != 'rejected') { 
				...
				if($result['distance'] == 0) { $distance =  '-'; } else { $distance = round($result['distance'],2);}
				if($result['fare'] == 0) { $fare =  '-'; } else { $fare =  round($result['fare'],2);}
				if($result['comments'] == '') { $comments =  'No Comments'; } else { $comments =  $result['comments'];}
			...
			if($list != 'rejected') { 
				if($result['transaction_id'] == ""){  $trans_id = '-';}else{ $trans_id = ucfirst($result['transaction_id']);}
				}
			$xls_output .= "<tr>"; 
			if($list != 'rejected') { 
				...
				else if($result['payment_type'] == 5 )
				{
				$xls_output .= '<td class="head_border">'.__('Wallet').'</td>';
				}	
				else
				{
				...
				if($result['trans_packtype'] == 'T' )
				{ 
					//$xls_output .= "<td>".__('transaction_based_commission')."</td>"; 	
				} 
				else if($result['trans_packtype'] == 'P' )
				{ 
					//$xls_output .= "<td>".__('package_based_commission')."</td>"; 	
				}
				else 
				{ 
					//$xls_output .= "<td>".__('package_based_no_commission')."</td>"; 	
				} 
			}
			if($list != 'rejected') { 
			...
			//$xls_output .= "<td>".wordwrap($result['email'],25,'<br />',1)."</td>"; 
			//$xls_output .= '<td class="head_border">'.strip_tags(htmlentities($result['current_location'])).'<br>'.strip_tags(htmlentities($result['drop_location'])).'</td>'; 
			...
			//$xls_output .= "<td>".$result['no_passengers']."</td>"; 
			if($list != 'rejected') { 
			...
			//$xls_output .= '<td class="head_border">'.$convet_amt.'</td>'; 
			//$xls_output .= "<td>".$result['nightfare']."</td>";
			/*$xls_output .= "<td>".$result['rating']."</td>"; 
			$xls_output .= "<td>".$comments."</td>"; */
			}
			elseif($list == 'cancelled') { 
			$xls_output .= '<td class="head_border">'.$fare.'</td>'; 
			$xls_output .= '<td class="head_border">'.$company_tax.'</td>'; 
			...
			$ccur_for = findcompany_currencyformat($company_currency);
			$convet_amt = currency_conversion($ccur_for,$tripfare);
			//$xls_output .= '<td class="head_border">'.round($convet_amt,2).'</td>'; 
			}
			else
			{
			$xls_output .= '<td class="head_border">'.$driver_reply.'</td>'; 
			//$xls_output .= '<td class="head_border">'.$driver_comments.'</td>'; 
			} 
			$xls_output .= "</tr>"; 
			if($list != 'rejected') { 
		...
		$xls_output .= "</table>"; 
			...
			//}
			/* if($list != 'rejected') {  
				$xls_output .= "<td>".__('subtotal')."</td>"; 
			}
			if($list != 'rejected' && $list != 'cancelled') { 
			$xls_output .= "<td>".CURRENCY.round($subtotal,2)."</td>";
			//$xls_output .= "<td>".$result['nightfare']."</td>";
			/*$xls_output .= "<td>".$result['rating']."</td>"; 
			$xls_output .= "<td>".$comments."</td>"; */
			/*}
	...
	public function action_settlement_list()
	{
		...
		if(isset($_POST['update']))
		{
			...
			{ */
				...
				$paypal_details = $this->commonmodel->payment_gateway_details();	
				$payment_gateway_username = isset($paypal_details[0]['payment_gateway_username'])?$paypal_details[0]['payment_gateway_username']:"";
				$payment_gateway_password = isset($paypal_details[0]['payment_gateway_password'])?$paypal_details[0]['payment_gateway_password']:"";
				$payment_gateway_key = isset($paypal_details[0]['payment_gateway_key'])?$paypal_details[0]['payment_gateway_key']:"";
				$currency_format = isset($paypal_details[0]['gateway_currency_format'])?$paypal_details[0]['gateway_currency_format']:"";
			...
			foreach($transaction_array as $key => $val)
			{
				...
				if($transaction->success == '1')
				{
					...
					$manage_transaction->update_table(TRANS,array('payment_status'=>str_replace('_',' ',$result->status)),'passengers_log_id',$trip_id);
		...
		$view = View::factory('admin/manage_settlement')
				->bind('Offset',$offset)
				->bind('action',$action)
