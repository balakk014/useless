		...
		switch($method)
		{
			...
			case 'check_companydomain':
				...
				if(count($result) > 0){
					...
					$baseurl = PROTOCOL."://".$company_domain.".".$mobiledata['company_main_domain']."/mobileapi118/index/";
										...
										{													
											...
											{	
												...
												if(count($get_passenger_log_details)>0)
												{
													...
													//check for two or threee drivers check
								...
								if(count($result) > 0)
								{
									...
									else
									{
										...
										if(count($getTaxiforDriver) > 0 )
										{
											...
											$alert_message =($driver_balance <=30)? "Your credits are too low. You will not receive trip request. Please recharge now!":"";
											
											$details['account_balance']=$driver_balance;
...
$fare_calculation_type = isset($array['fare_calculation_type']) ? $array['fare_calculation_type'] : FARE_CALCULATION_TYPE;
				...
				if(count($get_passenger_log_details) > 0)
				{
					...
					$account_balance = (isset($get_balance[0]['account_balance']) && $get_balance[0]['account_balance'] !="")?$get_balance[0]['account_balance']:0;
					$alert_message =($account_balance <=30)? "Your credits are too low. You will not receive trip request. Please recharge now!":"";
				...
				if(!empty($trip_array)) {
					...
					if($login_status == 1){
						...
						if(count($trip_list) > 0){
							...
							$alert_message =($driver_balance <=30)? "Your credits are too low. You will not receive trip request. Please recharge now!":"";
							$message = array("message" => __('drive_recent_trip_list'),"status" => 1,"alert_message"=>$alert_message,"account_balance"=>$driver_balance,"trip_list"=>$trip_list);
						}else{
			...
			case 'passenger_recharge_api':
				...
				if($validator->check()) 
				{
					...
					if(count($check_recharge_code)>0 && !empty($recharge_id)){
						...
						$message = array("message" => __('coupon_code_success'),"total_balance_amt"=>$account_balance,"past_history"=>$passenger_recharge_history,"voucher_amount"=>$recharge_amount,"currency"=>$this->site_currency,"status"=>1);
				...
				if(!empty($passenger_id)){
					...
					if(count($passenger_profile)>0){
						if(count($passenger_recharge_history)>0){
							$account_balance=isset($passenger_profile[0]['wallet_amount'])?$passenger_profile[0]['wallet_amount']:'0';
							$message = array("message" => __('success'),"total_balance_amt"=>$account_balance,"past_history"=>$passenger_recharge_history,"currency"=>$this->site_currency,"status"=>1);
						}else{
			...
			case 'driver_recharge_api':
				...
				if($validator->check()) 
				{
					...
					if(count($check_recharge_code)>0 && !empty($recharge_id)){
						...
						$alert_message =($account_balance <=30)? "Your credits are too low. You will not receive trip request. Please recharge now!":"";
						$message = array("message" => __('coupon_code_success'),"total_balance_amt"=>$account_balance,"past_history"=>$driver_recharge_history,"voucher_amount"=>$recharge_amount	,"currency"=>$this->site_currency,"status"=>1,"alert_message"=>$alert_message,"account_balance"=>$account_balance);
					}else if($check_recharge_code=='-1')
					{
				...
				if(!empty($driver_id)){
					...
					if(count($driver_profile)>0){
						if(count($driver_recharge_history)>0){
							$account_balance=isset($driver_profile[0]['account_balance'])?$driver_profile[0]['account_balance']:'0';
							$message = array("message" => __('success'),"total_balance_amt"=>$account_balance,"past_history"=>$driver_recharge_history,"currency"=>$this->site_currency,"status"=>1);
						}else{
							$message = array("message" => __('no_past_history'),"total_balance_amt"=>"0","status"=>2);
						}
					}else{
