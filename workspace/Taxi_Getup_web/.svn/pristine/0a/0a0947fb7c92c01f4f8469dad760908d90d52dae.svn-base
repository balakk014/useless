...
<script type="text/javascript" src="https://maps.google.com/maps/api/js?key=AIzaSyByFlLjgMjTCIoGEy_Et02p5aMV_XGl5jM&sensor=true"></script>
						...
						<div class="trip_det_lst">
							...
							<?php if($drop_location != "") { ?>
								<div class="passenger_drop_location">
									<p><?php echo $drop_location." ".$pickup_time; ?></p>
								</div>
							<?php } ?>
						</div>
			...
			<div class="fare_details">
				<div class="fare_det_lft">
					...
					<?php /*<ul>
						...
						<?php $subtot = $d['tripfare']+$d['taxi_waiting_cost']+$d['nightfare']+$d['eveningfare'];
							  $promotion = 0; ?>
						<?php if($d['passenger_discount'] != 0) { ?>
						<li>
							<label><?php echo __("Promotion").' ('.$d['passenger_discount'].'%)'; ?></label>
							<p>
								<?php 
									$promotion = ($subtot * $d['passenger_discount']) / 100;
									echo '- '.CURRENCY.round($promotion,2);
								?>
							</p>
						</li>
						<?php } ?>
						...
						<li><label><?php echo __("tax"); ?></label><p><?php echo ($d['company_tax'] != 0) ? CURRENCY.$d['company_tax'] : CURRENCY."0"; ?></p></li>
						<li>
                                                        <label>Paid Amount</label>
							<p><?php echo CURRENCY.round($d['used_wallet_amount'],2); ?></p>
						</li>
						<li class="tot_amt">
				...
				<div class="fare_det_rgt">
					...
					<form method="post" name="book_again_form">
						...
						<input type="hidden" name="pass_latitude" value="<?php echo $d['pickup_latitude']; ?>"/>
						<input type="hidden" name="pass_longitude" value="<?php echo $d['pickup_longitude']; ?>"/>
