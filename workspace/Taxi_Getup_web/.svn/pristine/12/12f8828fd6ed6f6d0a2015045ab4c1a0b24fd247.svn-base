...
<script src="<?php echo URL_BASE; ?>public/js/highcharts.js"></script>
<script src="<?php echo URL_BASE; ?>public/js/highcharts-3d.js"></script>
<script src="<?php echo URL_BASE; ?>public/js/exporting.js"></script>
	...
	<ul>
		...
		<select name="driver" >
			...
			<?php foreach($driver_list as $list) { ?>
				<option value="<?php echo $list['id'];?>"><?php echo $list['name']."-".$list['country_code'].$list['phone']; ?></option>
			<?php } ?>
	    ...
	    <li>
		<input type="button" name="search_total_details" onclick="dashboardTotalDetails();" value="GO" title="Go" >		
	    </li>
	</ul>
	</div>
    </div>
<?php /** Dashboard total details count Start * */ ?>
	...
	<div class="dashboard_widget_row">
		<div class="dashborad_widget_box lg-12">
			<div class="dashboard_widget_title with_calender">
				<h2><?php echo __('balance_no_balance_driver'); ?></h2>
					...
					<ul>
						<li>
							<div class="new_dash_calender_select">
								<select name="driver_id_one" id="driver_id_one">
									<option value=""><?php echo __('select_driver_label'); ?></option>
									<?php foreach($driver_list as $list) { ?>
										<option value="<?php echo $list['id'];?>"><?php echo $list['name']."-".$list['country_code'].$list['phone']; ?></option>
									<?php } ?>
						...
						<li>
							<input type="button" name="search_assign_unassign" onclick="driverbalanceChart();" value="GO" title="Go" >
						</li>
					</ul>
				</div>
				<?php } else { ?>
					<input type="hidden" id="driver_id_one" name="driver_id_one" value="0">
				<?php } ?>
			</div>
			<div class="dashboard_map_outer">
				<div class="balanceChartAjax"></div>
			</div>
		</div>
    </div>
    <div class="dashboard_widget_row company_wise_trip">
	   <?php if ($_SESSION['company_id'] == 0) { ?>
	<div class="dashborad_widget_box lg-6">
	 
	    <div class="dashboard_widget_title with_calender">
    		<h2><?php echo __('driver_trip_list'); ?></h2>
    		<div class="dashboard_calender_in_header with_select_box">
    		    <ul>

    		    <li>
							<div class="new_dash_calender_select">
								<select name="driver_id_one" id="driver_id_two">
									<option value=""><?php echo __('select_driver_label'); ?></option>
									<?php foreach($driver_list as $list) { ?>
										<option value="<?php echo $list['id'];?>"><?php echo $list['name']."-".$list['country_code'].$list['phone']; ?></option>
									<?php } ?>
    			...
    			<li>
    			    <input type="button" name="search_company_wise_trip" onclick="driverTripChart();" value="GO" title="Go" >
    			</li>
    		    </ul>
    		</div>    		
    	    </div>
    	    <div class="dashboard_map_outer">
    		<div class="driverTripChartAjax"></div>
    	    </div>
	...
	<div class="dashborad_widget_box <?php if ($_SESSION['company_id'] == 0) { ?> lg-6 <?php } else { ?> lg-12 <?php } ?>">
	    <div class="dashboard_widget_title with_calender">
		<h2><?php echo ($_SESSION['company_id'] == 0) ? __('driver_credits_label') : __('payment_label'); ?></h2>
		<div class="new_dash_calender_select pay_by_company" style="<?php if ($_SESSION['company_id'] > 0) { ?>display:none;<?php } ?> float:right;margin-top:-10px;">
				<select name="driver_id_three" id="driver_id_three" >
				    <option value=""><?php echo __('select_driver_label'); ?></option>
					<?php foreach($driver_list as $list) { ?>
						<option value="<?php echo $list['id'];?>"><?php echo $list['name']."-".$list['country_code'].$list['phone']; ?></option>
					<?php } ?>
		...
		<div class="dashboard_calender_in_header with_select_box payment_box payment_by_company" style="margin-top:11px;">
			...
			<li>
			    <input type="button" name="search_payment_by_company" onclick="drivertopup();" value="GO" title="Go" >
			</li>
		    </ul>
		</div>
		</div>
	    <div class="dashboard_map_outer">
		<div class="drivertopupAjax <?php if ($_SESSION['company_id'] > 0) { ?>payment_by_company<?php } ?>"></div>
 ...
 <?php /** City By Count Start * */ ?>
	...
	<div class="dashborad_widget_box lg-12">
	    <div class="dashboard_widget_title with_calender">
		<h2><?php echo __('online_vs_offline'); ?></h2>
		<div class="dashboard_calender_in_header with_select_box trip_req_from_city">
		    <ul>
			<li <?php if ($_SESSION['company_id'] > 0) { ?>style="display:none;"<?php } ?>><div class="new_dash_calender_select">
				<select name="driver_id_four" id="driver_id_four" >
						<option value=""><?php echo __('select_driver_label'); ?></option>
					<?php foreach($driver_list as $list) { ?>
						<option value="<?php echo $list['id'];?>"><?php echo $list['name']."-".$list['country_code'].$list['phone']; ?></option>
					<?php } ?>
				</select></div></li>
			<li style="display:none"><div class="new_dash_calender_input">
		    ...
		    <span id="city_by_count_enddate_error" class="errors" style="display:none;"></span>
			    ...
			    <li>
				<input type="button" name="search_city_by_count" onclick="onlineOfflineChart();" value="GO" title="Go" >
			    </li>
		    </ul>
		</div>
	    ...
	    <div class="dashboard_map_outer">
		<div class="onlineOfflineAjax"></div>
	    </div>
	</div>	    
    </div>
...
<?php /** Company Revenue Graph Start * */ ?>
	...
	<div class="dashborad_widget_box lg-12">
	    ...
	    <div class="dashboard_map_outer">
		<div class="companyRevenueChartAjax"></div>
	    </div>
	</div>	    
    </div> */ ?>
...
<?php /** Driver Revenue Graph Start * */ ?>
	...
	<div class="dashborad_widget_box lg-12">
	    <div class="dashboard_widget_title with_calender">
		<h2><?php echo __('driver_trip_details_label'); ?></h2>
		<div class="dashboard_calender_in_header with_select_box driver_revenue">
			...
			<li><input type="button" name="search_driver_revenue" onclick="driverRevenueChart();" value="GO" title="Go" ></li>
		    </ul>
		</div>		
	    </div>
	    <div class="dashboard_map_outer">
		<div class="driverRevenueChartAjax"></div>
	    </div>
	</div>	    
    </div>
...
<script type="text/javascript" language="javascript">
	...
	var select_driver = $("select[name=driver]").val();
	var driver = (select_driver !="") ? select_driver : "";
	...
	if(startdate !='' && enddate!='') {
	    $('.dashboardTotalDetailsAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
		...
		$("#ttstartdate_error").html("");
		$("#ttstartdate_error").hide();
		var dataS = "startdate="+startdate+"&enddate="+enddate+"&driver="+driver;
		$.ajax ({
		    type: "POST",
		    url: SrcPath+"dashboard/driver_dashboardTotalDetails",
		    data: dataS, 
		    cache: false, 
		    dataType: 'html',
	...
	function driverbalanceChart()
	{
		var select_company = $("#driver_id_one").val();
		$('.balanceChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');
		var data = "company="+select_company;
		$.ajax ({
			type: "POST",
			url: SrcPath+"dashboard/driverbalanceChart",
			cache: false, 
			dataType: 'html',
			data: data,
			success: function(response) 
			{
			$('.balanceChartAjax').html(response);
			} 
		});
	}

    function driverTripChart()
    {
	...
	if(startdate !='' && enddate!='') {
	    $('.driverTripChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
	    if(to_timestamp(startdate) > to_timestamp(enddate)) {
		...
		$("#company_wise_enddate_error").hide();
		var driver_id_two = $("#driver_id_two").val();
		var dataS = "startdate="+startdate+"&enddate="+enddate+"&driver_id_two="+driver_id_two;
		$.ajax ({
		    type: "POST",
		    url: SrcPath+"dashboard/dashboardDriverTripChart",
		    data: dataS, 
		    cache: false, 
		    dataType: 'html',
		    success: function(response) 
		    {
			$('.driverTripChartAjax').html(response);
		    } 
		});
	    }
	}
	...
	if(startdate !='' && enddate!='') {
	    $('.drivertopupAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
	    if(to_timestamp(startdate) > to_timestamp(enddate)) {
		...
		$.ajax ({
		    type: "POST",
		    url: SrcPath+"dashboard/drivertopup",
		    data: dataS, 
		    cache: false, 
		    dataType: 'html',
		    success: function(response) 
		    {
			$('.drivertopupAjax').html(response);
		    } 
		});
	    }
	}
    ...
    /* function companyCountChart()
{
	...
	if(startdate !='' && enddate!='') {
		$('.companyCountChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
		if(to_timestamp(startdate) > to_timestamp(enddate)) {
    ...
    /* function tripReqTopCitiesChart()
{
	...
	if(startdate !='' && enddate!='') {
		$('.tripReqTopCitiesChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
		if(to_timestamp(startdate) > to_timestamp(enddate)) {
	...
	if(startdate !='' && enddate!='') {
	    $('.onlineOfflineAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
	    if(to_timestamp(startdate) > to_timestamp(enddate)) {
		...
		$.ajax ({
		    type: "POST",
		    url: SrcPath+"dashboard/onlineOfflineChart",
		    data: dataS, 
		    cache: false, 
		    dataType: 'html',
		    success: function(response) 
		    {
			$('.onlineOfflineAjax').html(response);
		    } 
		});
	    }
	}
	...
	function companyRevenueChart()
	{
		...
		if(company == "") { brand_filter = 0; } else if(label == "Multy") { brand_filter = 1; } else if(label == "Single") { brand_filter = 2; }
		...
		if(startdate !='' && enddate!='') {
			$('.companyRevenueChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
			if(to_timestamp(startdate) > to_timestamp(enddate)) {
	...
	function driverRevenueChart()
	{
		...
		if(startdate !='' && enddate!='') {
			$('.driverRevenueChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
			if(to_timestamp(startdate) > to_timestamp(enddate)) {
			...
			$.ajax ({
				type: "POST",
				url: SrcPath+"dashboard/dashboardDriverDetailsChart",
				data: dataS, 
				cache: false, 
				dataType: 'html',
				success: function(response) 
