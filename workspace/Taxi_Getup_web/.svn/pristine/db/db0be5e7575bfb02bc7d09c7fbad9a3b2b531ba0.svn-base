					...
					<table cellspacing="1" cellpadding="10" width="100%" align="center" class="sTable responsive">
						...
						<tbody>
							...
							<tr class="<?php echo $trcolor; ?>">
								...
								<td align="center"> 
									<?php 
										if($listings['payment_mode_status'] == 1) {
											$txt = "unsuspendicon"; $class ="blockicon";
										} else { 
	...
	$('#more_action').change(function() {
		...
		switch (selected_val) {
			...
			if($('input[type="checkbox"]').is(':checked')) {
				...
				if(ans) {
					document.manage_payment.action="<?php echo URL_BASE;?>manage/block_unblock__withdraw_payment/1";
			...
			case "active_country_request":
				...
				if($('input[type="checkbox"]').is(':checked')) {
					var ans = confirm(confirm_msg)
					if(ans) {
						document.manage_payment.action="<?php echo URL_BASE;?>manage/block_unblock__withdraw_payment/0";
						document.manage_payment.submit();
					} else {
						$('#more_action').val('');
					}
