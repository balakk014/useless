	...
	<?php } ?>
		...
		<div class="widget" style="margin-bottom:0px !important;">
			<div class="title"><h6><?php echo $page_title; ?></h6>
				<div style="width:auto; float:right; margin: 0 3px;"><?php if($count_transaction_list > 0){ $export_table_count = $count_transaction_list; include_once(APPPATH.'views/admin/transaction_export_menu.php'); } ?></div>
			</div>
		</div>
...
<tbody>	
		...
		<tr class="<?php echo $trcolor; ?>">
			...
			<td><?php if($listings['transaction_id'] != "")
				...
				<?php } else { ?>
				- <?php } ?></td>
			<td><?php if($listings['payment_type'] == 2) { echo __('credit_card_using').$listings['payment_gateway_name'].' ('.$paymentMod.')'; } else if($listings['payment_type'] == 4) { echo __('account'); } else if($listings['payment_type'] == 3){ echo __('new_credit_card').' ('.$paymentMod.')'; } else if($listings['payment_type'] == 5) { echo __('Wallet'); } else { echo __('cash'); } ?></td>
			<?php } ?>
			<td>
				<?php if($list != 'rejected') { ?>
