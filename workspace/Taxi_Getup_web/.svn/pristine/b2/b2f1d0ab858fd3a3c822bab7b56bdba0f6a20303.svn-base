...
<div class="container_content fl clr">
	...
	<tr>
	<?php 
		if(isset($state_details[0]['state_countryid'])){
			$country_name = $state_details[0]['state_countryid'];
		}else{
			if(isset($postvalue['country_name'])){
				$country_name = $state_details['country_name'];
			}else{
				$country_name = "";
			}
		}
	...
	<?php 
		if(isset($state_details[0]['state_name'])){
			...
			<td>&nbsp;</td>
                        <td colspan="">
                            ...
                            <div class="new_button"><input type="button" value="<?php echo __('button_back'); ?>" onclick="window.history.go(-1)" /></div>
							 <div class="new_button"><input type="submit" value="<?php echo __('submit' );?>" name="submit_editstate" title="<?php echo __('submit' );?>" /></div>
