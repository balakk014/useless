...
class Controller_TaximobilityManage extends Controller_Siteadmin 
{
	...
	*/		
	...
	{	
		...
		//$count_company_list = $manage_company->count_company_list();
		...
		//$all_company_list = $manage_company->all_company_list($offset, REC_PER_PAGE);
		...
		$view = View::factory('admin/manage_company')
	...
	{	
		...
		//,trim(Html::chars($_REQUEST['filter_company']))
		...
		if(isset($_SESSION['download_set'])){
			...
			$export_table_header= array(__('name'),__('companyname'),__('email_label'),__('no_of_taxi'), __('no_of_driver'),__('no_of_manager'),__('Status'));
			$export_table_field_select= array('name','company_name','email','no_of_taxi','no_of_driver','no_of_manager','company_status');
	...
	$this->is_login(); 
	   ...
	   $res_com = $manage->get_front_company_request($_REQUEST['uniqueId']);
		...
		foreach($res_com as $rc)
		{
			...
			$mail="";
			$replace_variables=array(REPLACE_LOGO=>EMAILTEMPLATELOGO,REPLACE_SITENAME=>$this->app_name,REPLACE_USERNAME=>ucfirst($rc['name']),REPLACE_EMAIL=>$rc['email'],REPLACE_PASSWORD=>$rc['org_password'],REPLACE_SITELINK=>URL_BASE.'users/contactinfo/',REPLACE_SITEEMAIL=>$this->siteemail,REPLACE_SITEURL=>URL_BASE,REPLACE_COPYRIGHTS=>SITE_COPYRIGHT,REPLACE_COPYRIGHTYEAR=>COPYRIGHT_YEAR);
		...
		//$this->request->redirect("manage/company");	 //transaction/index
	...
	public function action_motor()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_motorcompany')
				->bind('all_company_list',$all_company_list)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	public function action_model()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_motormodel')
				->bind('all_company_list',$all_company_list)
			    ->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	public function action_fare()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_fare')
				->bind('all_fare_list',$all_fare_list)
			    ->bind('pag_data',$pag_data)
				->bind('srch',$_REQUEST)
            ->bind('Offset',$offset);
	...
	public function action_companymodelsearch()
	{
		...
		$count_fare_list = $manage_fare->count_company_searchmodel_list(trim(Html::chars($_REQUEST['keyword'])),trim(Html::chars($_REQUEST['status'])));
		...
		//Post results for search 
        if($_REQUEST){
			$all_fare_list = $manage_fare->get_company_all_model_searchlist(trim(Html::chars($_REQUEST['keyword'])),trim(Html::chars($_REQUEST['status'])),$offset, REC_PER_PAGE);
		}
		//set data to view file	
    	$view = View::factory('admin/manage_fare')
				->bind('title',$title)
	...
	public function action_modelinfo()
	{
		...
		//send data to view file 
		$view=View::factory(ADMINVIEW.'manage_motormodelinfo')
	...
	public function action_active_model_request()
	{
		...
		//redirects to job_feedback details page after deletion
		//$this->request->redirect("manage/model");	 //transaction/index
	...
	public function action_driver()
	{
		...
		$availabilitycount = $add_company->validate_packagedriver($cid);
		...
		//send data to view file 
		$view = View::factory('admin/manage_driver')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    ->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('availabilitycount',$availabilitycount)
				->bind('srch',$_REQUEST)
	            ->bind('Offset',$offset)
	            ->bind('all_list',$all_list);
	...
	public function action_driversearch()
	{
		...
		$availabilitycount = $add_company->validate_packagedriver($cid);
		if($usertype!='A'){
			$count_company_list = $manage_company->count_searchdriver_list(trim(Html::chars($_REQUEST['keyword'])),trim(Html::chars($_REQUEST['status'])),trim(Html::chars($company_id)),trim(Html::chars($_REQUEST['shift_status'])), $peoples,$driver_list);
			$all_list=$count_company_list;
			$count_company_list=count($count_company_list);
		}else{
			$count_company_list = $manage_company->count_searchdriver_list(trim(Html::chars($_REQUEST['keyword'])),trim(Html::chars($_REQUEST['status'])),trim(Html::chars($_REQUEST['filter_company'])),trim(Html::chars($_REQUEST['shift_status'])), $peoples,$driver_list);
			$all_list=$count_company_list;
			$count_company_list=count($count_company_list);
		}
		...
		if(isset($_SESSION['download_set'])){
			...
			if($usertype == 'A') {
				$export_table_header= array(__('name'),__('email_label'),__('companyname'),__('phone_label'), __('country_label'),__('state_label'),__('city_label'),__('driver_license_id'),__('Account Balance'),__('driver_status'),__('last_update'));
				$export_table_field_select= array('name','email','company_name','phone','country_name','state_name','city_name','driver_license_id','account_balance','shift_status','update_date');
			} else {
				$export_table_header= array(__('name'),__('email_label'),__('phone_label'), __('country_label'),__('state_label'),__('city_label'),__('driver_license_id'),__('Account Balance'),__('driver_status'),__('last_update'));
				$export_table_field_select= array('name','email','phone','country_name','state_name','city_name','driver_license_id','account_balance','shift_status','update_date');
			}
			$heading='taxiexport';
			$this->action_create_the_document($all_list,$export_table_header,$export_table_field_select,$heading);
		}
		...
		if($_REQUEST){
			if($usertype!='A'){
				$all_company_list = $manage_company->get_all_driver_searchlist(trim(Html::chars($_REQUEST['keyword'])),trim(Html::chars($_REQUEST['status'])),trim(Html::chars($company_id)),trim(Html::chars($_REQUEST['shift_status'])),$offset, REC_PER_PAGE, $peoples,$driver_list);
			}else{
				$all_company_list = $manage_company->get_all_driver_searchlist(trim(Html::chars($_REQUEST['keyword'])),trim(Html::chars($_REQUEST['status'])),trim(Html::chars($_REQUEST['filter_company'])),trim(Html::chars($_REQUEST['shift_status'])),$offset, REC_PER_PAGE, $peoples,$driver_list);
			}
		}
		$get_allcompany = $manage_company->get_allcompany();
		//set data to view file	
	...
	public function action_active_driver_request()
	{
		...
		if($usertype !='A')
		{
			$check_result = $add_model->validate_packagedriver($company_id);
			...
			$check_result = $add_model->validate_package_assigntaxi($company_id);
	...
	{	
		...
		//send data to view file 
		$view = View::factory('admin/manage_field')
				->bind('all_company_list',$all_company_list)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	{	
		...
		$availabilitycount = $add_company->validate_packagetaxi($cid);
		...
		//send data to view file 
		$view = View::factory('admin/manage_taxi')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    ->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				...
				->bind('srch',$_REQUEST)
	            ->bind('Offset',$offset)
	            ->bind('all_list',$count_company_list);
	...
	public function action_taxisearch()
	{
		...
		if(isset($_SESSION['download_set'])){
			if($usertype == 'A') {
				$export_table_header= array(__('taxi_no'),__('companyname'),__('taxi_type'),__('taxi_model'),__('Status'));
				$export_table_field_select= array('taxi_no','company_name','motor_name','model_name','taxi_status');
			} else {
				$export_table_header= array(__('taxi_no'),__('taxi_type'),__('taxi_model'),__('Status'));
				$export_table_field_select= array('taxi_no','motor_name','model_name','taxi_status');
			}
			$heading='ManageTaxi';
			$this->action_create_the_document($all_list,$export_table_header,$export_table_field_select,$heading);
		}
	...
	public function action_active_taxi_request()
	{
		...
		if($usertype !='A')
		{
			$check_result = $add_model->validate_packagetaxi($company_id);
		...
		//redirects to job_feedback details page after deletion
		//$this->request->redirect("manage/taxi");	 //transaction/index
		$this->request->redirect($_SERVER['HTTP_REFERER']);
	...
	public function action_package()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_package')
				->bind('all_company_list',$all_company_list)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	public function action_country()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_country')
				->bind('all_company_list',$all_company_list)
				->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	public function action_active_country_request()
	{
		...
		//redirects to job_feedback details page after deletion
		//$this->request->redirect("manage/country");	 //transaction/index
		$this->request->redirect($_SERVER['HTTP_REFERER']);
	...
	public function action_city()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_city')
				->bind('all_company_list',$all_company_list)
				->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	public function action_state()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_state')
				->bind('all_state_list',$all_state_list)
				->bind('pag_data',$pag_data)
				->bind('StateList',$StateList)
				->bind('srch',$_REQUEST)
				->bind('Offset',$offset);
	...
	{	
		...
		//send data to view file 
		$view = View::factory('admin/manage_manager')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    ->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
	...
	public function action_managersearch()
	{
		...
		if(isset($_SESSION['download_set'])){
			...
			if($usertype == 'A'){
				$export_table_header= array(__('name'),__('companyname'),__('email_label'),__('address'), __('country_label'),__('state_label'),__('city_label'),__('Status'));
				$export_table_field_select= array('name','company_name','email','address','country_name','state_name','city_name','status');
			} else {
				$export_table_header= array(__('name'),__('email_label'),__('address'), __('country_label'),__('state_label'),__('city_label'),__('Status'));
				$export_table_field_select= array('name','email','address','country_name','state_name','city_name','status');
			}
	...
	public function action_admin()
	{
		...
		$details ='';
		//send data to view file 
		$view = View::factory('admin/manage_admin')
				->bind('all_admin_list',$all_admin_list)
				->bind('pag_data',$pag_data)
				->bind('srch',$_REQUEST)
				->bind('Offset',$offset)
				->bind('all_list',$all_list);
	...
	public function action_assigntaxi()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_assigntaxi')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
				->bind('pag_data',$pag_data)
				//->bind('CompanyList',$CompanyList)
	...
	public function action_assigntaxisearch()
	{
		...
		if($usertype !='A')
		{
			$check_result = $add_company->validate_packagetaxi($cid);
			...
			$check_result = $add_company->validate_packagedriver($cid);
			...
			$check_result = $add_company->validate_package_assigntaxi($cid);
		...
		if(isset($_SESSION['download_set'])) {
			...
			if($usertype == 'A'){
				$export_table_header= array(__('driver_name'),__('taxi_no'),__('companyname'), __('country_label'),__('state_label'),__('city_label'),__('from_date'),__('end_date'),__('Status'));
				$export_table_field_select= array('name','taxi_no','company_name','country_name','state_name','city_name','mapping_startdate','mapping_enddate','mapping_status');
			} else {
				$export_table_header= array(__('driver_name'),__('taxi_no'), __('country_label'),__('state_label'),__('city_label'),__('from_date'),__('end_date'),__('Status'));
				$export_table_field_select= array('name','taxi_no','country_name','state_name','city_name','mapping_startdate','mapping_enddate','mapping_status');
			}
	...
	public function action_unassign_taxi()
	{
		...
		if(count($status) > 0){
			...
			//if(count($_REQUEST['driverId']) > 0){
				//function to get google api key for push notification(for android)
				$customer_google_api = $commMdl->select_site_settings('driver_android_key',SITEINFO);
				foreach($status as $mapId => $driverId) {
					//if(in_array($mapId,$_REQUEST['uniqueId'])){
						...
						//print_r($driverDeviceDets);exit;
						if(count($driverDeviceDets) > 0 && !empty($driverDeviceDets[0]['device_token'])){
							$pushMessage = array("message" => __('taxi_unassigned_by_admin'),"status"=>15,"display"=>1);
	...
	public function action_ratingdrivers()
	{
		...
		//send data to view file 
		$view = View::factory('admin/managerating_drivers')
					->bind('all_rating_drivers',$all_rating_drivers)
					->bind('pag_data',$pag_data)
					->bind('RatingdriversList',$RatingdriversList)
					->bind('get_rate_company',$get_rate_company)
					->bind('Offset',$offset);
	...
	public function action_managerating_driversview()
	{
		...
		//send data to view file 
		$view = View::factory('admin/managerating_driversview')
				->bind('all_rating_drivers',$all_rating_drivers)
				->bind('pag_data',$pag_data)
				->bind('driver_profile',$driver_profile)
				->bind('Offset',$offset);
	...
	public function action_ratingcompanies()
	{
		...
		//send data to view file 
		$view = View::factory('admin/managerating_companies')
				->bind('all_rating_companies',$all_rating_companies)
			    	->bind('pag_data',$pag_data)
				->bind('RatingcompanyList',$RatingcompanyList)
            			->bind('Offset',$offset);
	...
	public function action_packagereport()
	{
		...
		//send data to view file 
		$view = View::factory('admin/package_report')
				->bind('all_company_list',$all_company_list)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	            		->bind('Offset',$offset)
				->bind('rcompany_list',$rcompany_list);
	...
	public function action_packagereports()
	{
		...
		//send data to view file 
		$view = View::factory('admin/package_reports')
				->bind('all_company_list',$all_company_list)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
				->bind('srch',$_REQUEST)
	...
	public function action_contacts_search()
	{
		...
		//set data to view file	
		$view = View::factory('admin/manage_contacts')
				->bind('title',$title)
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
				->bind('ContactsList',$ContactsList)
				->bind('all_contacts_list',$all_contacts_list);
	...
	public function action_contact_view()
	{
		...
		if($usertype =='C'&&$cid!=$ContactsList[0]['contact_cid'])
		{
			$this->request->redirect("manage/contacts");
		}		
		
		$view = View::factory('admin/manage_contact_view')
			    	->bind('pag_data',$pag_data)
				->bind('ContactsList',$ContactsList)
            			->bind('Offset',$offset)
	...
	public function action_free_quotes_search()
	{
		...
		$search_post = arr::get($_REQUEST,'search_user');
		if($usertype !='C')
		$count_free_quotes_list = $manage_contacts->get_all_free_quotes_searchlist_count(trim(Html::chars($_REQUEST['keyword'])));
		//pagination loads here
		$page_no= isset($_GET['page'])?$_GET['page']:0;
		if($page_no==0 || $page_no=='index')
	        $page_no = PAGE_NO;
		...
		if(isset($search_post) && $_REQUEST){
			if($usertype !='C')
			$all_free_quotes_list = $manage_contacts->get_all_free_quotes_searchlist(trim(Html::chars($_REQUEST['keyword'])));
			
		}
		//set data to view file	
		$view = View::factory('admin/manage_free_quotes')
				->bind('title',$title)
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
				->bind('ContactsList',$ContactsList)
				->bind('all_free_quotes_list',$all_free_quotes_list);
	...
	public function action_free_quotes()
	{
		...
		$manage_contacts = Model::factory('manage');
                if($usertype !='C')
		$count_free_quotes_list = $manage_contacts->count_free_quotes_list();
		...
		'view' => 'pagination/punbb',
                 ...
                 if($usertype !='C')
		$all_free_quotes_list = $manage_contacts->all_free_quotes_list($offset, REC_PER_PAGE);
	...
	public function action_free_quotes_view()
	{
		...
		$id = $this->request->param('id');
		$manage_contacts = Model::factory('manage');
		$ContactsList = $manage_contacts->free_quotes_list_view($id);
		if($usertype =='C'&&$cid!=$ContactsList[0]['id'])
		{
			$this->request->redirect("manage/free_quotes");
		}		
		$view = View::factory('admin/manage_free_quotes_view')
			    	->bind('pag_data',$pag_data)
				->bind('ContactsList',$ContactsList)
            			->bind('Offset',$offset)
            			->bind('usertype',$usertype);
	...
	public function action_delete_free_quotes()
	{
		$id = $this->request->param( 'id' );
		$delete_contacts = Model::factory('manage');
		$contacts = $delete_contacts->delete_free_quotes($id);
		if($contacts){
	...
	public function action_userinfo()
	{
		...
		$view = View::factory('admin/userinfo')
				->bind('pag_data',$pag_data)
	...
	public function action_admin_userinfo()
	{
		...
		$view = View::factory('admin/admin_userinfo')
				->bind('pag_data',$pag_data)
				->bind('user_details',$user_details)
				->bind('Offset',$offset);
	...
	public function action_passengerinfo()
	{
		...
		/*$user_group_details = $view_controller->details_passenger_group_info($id);
		if($_SESSION['user_type'] == 'C')
		{
			$acc_id = isset($user_group_details[0]['aid'])?$user_group_details[0]['aid']:'0';
			$user_account_details = $view_controller->details_passenger_account_info($acc_id);
		}*/
		
		
		/***** Completed Transaction *********************/
		//$count_passenger_logs_completed_transaction = $view_controller->get_passenger_completed_transaction($id,'R','A','1',REC_PER_PAGE,0,1,'','');
		$passenger_logs_completed_transaction = $view_controller->get_passenger_completed_transaction($id,'R','A','1',REC_PER_PAGE,0,1,'','');

		/***** ongoing Transaction *********************/
		$passenger_logs_inprogress = $passengers_model->get_passenger_ongoing_transaction($id,1,"","");
	
		/**************************************************/		
		$view = View::factory('admin/passengerinfo')
				->bind('pag_data',$pag_data)
				->bind('user_details',$user_details)
				//->bind('user_group_details',$user_group_details)
				->bind('user_account_details',$user_account_details)
				->bind('passenger_logs_inprogress',$passenger_logs_inprogress)
				->bind('passenger_logs_completed_transaction',$passenger_logs_completed_transaction)
				//->bind('count_passenger_logs_completed_transaction',$count_passenger_logs_completed_transaction)
				->bind('Offset',$offset);
	...
	public function action_companyinfo()
	{
		...
		$view = View::factory('admin/companyinfo')
			    	->bind('pag_data',$pag_data)
				->bind('user_details',$user_details)
            			->bind('Offset',$offset);
	...
	public function action_driverinfo()
	{
		...
		//echo '<pre>';print_r($user_details1);exit;
		if(count($user_details1) == 0) {
			$this->urlredirect->redirect('manage/driver');
		}
		
		$dashstyles = array('public/css/dashboard/bootstrap-responsive.css' =>'screen',
						'public/css/dashboard/charisma-app.css' =>'screen',
						'public/css/dashboard/jquery-ui-1.8.21.custom.css' =>'screen',
						'public/css/dashboard/chosen.css' =>'screen',
						'public/css/dashboard/uniform.default.css' =>'screen',
						'public/css/dashboard/jquery.noty.css' =>'screen',
						'public/css/dashboard/noty_theme_default.css' =>'screen',
						'public/css/dashboard/jquery.iphone.toggle.css' =>'screen',
						'public/css/dashboard/opa-icons.css' =>'screen',
						'public/css/datepicker.css'=>'screen');
		...
		//$pmid = $manage->check_peoplecompanyid($id);exit;
		...
		//$count_user_details = count($view_controller->get_driver_logs1($id,'R','A','1'));exit;
		$user_details = $view_controller->get_driver_logs1($id,'R','A','1',REC_PER_PAGE,'0');
		# Getting all Completed driver logs
		//$driver_logs_completed = $view_controller->get_driver_logs_completed($id,'R','A','1',REC_PER_PAGE,'0');
		//$count_driver_logs_completed_transaction = count($manage->get_driver_completed_transaction($id,'R','A','1',REC_PER_PAGE,0,1,'',''));
		//$count_driver_logs_service = $view_controller->count_get_driver_logs_service($id);			
		//$driver_logs_service = $view_controller->get_driver_logs_service($id,REC_PER_PAGE,'0');	
		...
		//$driver_logs_progress = $view_controller->get_driver_logs($id,'R','A','2',$driver_cmid);
		 
		//Getting Up Coming Data
		//$driver_logs_upcoming = $view_controller->get_driver_logs($id,'R','A','0',$driver_cmid);	
		...
		if(!empty($driver_logs_progress_upcoming)){
			foreach($driver_logs_progress_upcoming as $up){
				...
				if($up->travel_status == 2){
					$driver_logs_progress[] = $up;
				}

				if($up->travel_status == 3){
					$driver_logs_progress[] = $up;
				}
				if($up->travel_status == 5){
					$driver_logs_progress[] = $up;
				}
			}
		}
		...
		$view = View::factory('admin/driverinfo')
					->bind('pag_data',$pag_data)
					//->bind('count_user_details',$count_user_details)
					//->bind('count_driver_logs_service',$count_driver_logs_service)
					//->bind('driver_logs_service',$driver_logs_service)
					//->bind('driver_logs_completed',$driver_logs_completed)
					...
					->bind('get_tot_trans_driver',$get_tot_trans_driver)
					->bind('get_tot_ratings_driver',$get_tot_ratings_driver)
					->bind('get_trip_statitics',$get_trip_statitics)
					->bind('total_trip_statitics',$total_trip_statitics)
					...
					->bind('count_get_driver_shift_logs',$count_get_driver_shift_logs)
					...
					->bind('driverWithdrawReqs',$driverWithdrawReqs)
	...
	{ 
		...
		else
		{
			...
			//print_r($passengers_details);exit;
...
$Tophtml .= '
 ...
 $Tophtml .= '</table>';
			...
			$Middle_html .= '<table border="0" cellpadding="1" cellspacing="1" width="700">
						...
						($i%2 == 1)?$class="eventr":$class="oddtr";
							...
							foreach($driver_logs_completed_transaction as $values)
							{
								...
								$company_currency = CURRENCY;
							if($travel_status == 0) { $status = __('critical'); $row_solor = 'style="color:#00FF00;"';  } elseif($travel_status == 1) { $status = __('completed'); $row_solor = 'style="color:#00FF00;"'; }  elseif($travel_status == 2) { $status = __('inprogress'); $row_solor = 'style="color:#0000FF;"'; }  if($travel_status == 3) { $status = __('start_to_pickup'); $row_solor = 'style="color:#FFFF00;"'; } elseif($travel_status == 4) { $status = __('cancel_by_passenger'); $row_solor = 'style="color:#990066;"';} elseif($travel_status== 5) { $status = __('waiting_payment'); $row_solor = 'style="color:#00FFFF;"';} elseif($travel_status == 6) { $status = __('missed'); $row_solor = 'style="color:#FF6633;"';} elseif($travel_status == 7) { $status = __('dispatched'); $row_solor = 'style="color:#003333;"'; }  elseif($travel_status == 8) { $status = __('cancelled'); $row_solor = 'style="color:#990000;"';} 
							$Middle_html .= '<tr>	
								<td width=10>'.$i.'</td>
	...
	public 	function action_driverlogs()
	{
		...
		$view = View::factory('admin/driverlogs')
				->bind('pag_data',$pag_data)
				->bind('driver_logs_service',$driver_logs_service);
	...
	public 	function action_drivershifthistory()
	{
		...
		$view = View::factory('admin/drivershifts')
				->bind('pag_data',$pag_data)
				->bind('Offset',$offset)
				->bind('driver_shift_logs',$driver_shift_logs);
	...
	public 	function action_taxilogs()
	{
		...
		$view = View::factory('admin/taxilogs')
				->bind('pag_data',$pag_data)
				->bind('taxi_logs_service',$taxi_logs_service);
	...
	public function action_taxiinfo()
	{
		...
		//$taxi_driver = $view_controller->details_taxi_driver($taxi_details[0]['taxi_id']);exit; 
		$taxi_driver = $view_controller->mapping_driver_details($taxi_details[0]['taxi_id']);
		
		if($taxi_driver){
			//$user_details1 = $view_controller->details_userinfo($taxi_driver[0]['driverid']);
			$user_details1 = $taxi_driver;
		}

		//$additional_fields = $view_controller->taxi_additionalfields();

		/*$count_taxi_logs_service = $view_model->count_get_taxi_logs_service($id);					
		$taxi_logs_service = $view_model->get_taxi_logs_service($id,REC_PER_PAGE,0);			*/
		...
		$view = View::factory('admin/taxiinfo')
				->bind('pag_data',$pag_data)
				->bind('additional_fields',$additional_fields)
				->bind('taxi_details',$taxi_details)
				->bind('taxi_driver',$taxi_driver)
				->bind('user_details1',$user_details1)
				->bind('count_taxi_logs_service',$count_taxi_logs_service)
				->bind('taxi_logs_service',$taxi_logs_service)
				->bind('count_taxi_logs_completed_transaction',$count_taxi_logs_completed_transaction)
				->bind('taxi_logs_completed_transaction',$taxi_logs_completed_transaction)
				->bind('Offset',$offset)
				->bind('company_currency',$company_currency)
	...
	public function action_contents()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_content')
				->bind('all_content_list',$all_content_list)
			    	->bind('pag_data',$pag_data)
			    	->bind('Offset',$offset)
				->bind('ContentList',$ContentList)
            			->bind('Offset',$offset);
	...
	public function action_company_contents()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_company_content')
				->bind('all_content_list',$all_content_list)
			    	->bind('pag_data',$pag_data)
			    	->bind('Offset',$offset)
				->bind('ContentList',$ContentList)
	...
	public function action_content_view()
	{
		...
		$view = View::factory('admin/manage_content_view')
			    	->bind('pag_data',$pag_data)
				->bind('ContactsList',$ContactsList)
            			->bind('Offset',$offset);
	...
	public function action_content_edit_view()
	{
		...
		//$manage_edit = Model::factory('edit');
		...
		if ($signup_submit && Validation::factory($_POST) ) 
		{
			...
			$validator = $manage_content->validate_editview(arr::extract($post_values,array('menu_name','meta_title','meta_keyword','meta_description')));
		...
		$view = View::factory('admin/edit_content_view')
				->bind('errors',$errors)
			    	->bind('pag_data',$pag_data)
			    	->bind('postvalue',$postvalues)
				->bind('company_details',$ContactsList)
            			->bind('Offset',$offset)
	...
	public function action_company_content_edit()
	{
		...
		if ($signup_submit && Validation::factory($_POST) ) 
		{
			...
			$validator = $manage_content->validate_companyeditview(arr::extract($post,array('menu_name','page_title','page_url')),$cid,$id);
		...
		$ContactsList = $manage_content->company_content_list_view($id);
		$view = View::factory('admin/edit_content_company')
				->bind('errors',$errors)
			    	->bind('pag_data',$pag_data)
			    	->bind('postvalue',$postvalues)
				->bind('company_details',$ContactsList)
	...
	public function action_companydetails()
	{
		...
		if(!empty($package_details)){
			foreach($package_details as $p){
				$company_info[] = array('company_domain' => $p['company_domain'], 'company_currency' => $p['company_currency']);
			}
		}
		//$company_info = $view_controller->company_info($cmid[0]['cid']);	
		
		$view = View::factory('admin/companydetails')
			    	->bind('pag_data',$pag_data)
				->bind('user_details',$user_details)
				->bind('id',$id)
	...
	public function action_managerdetails()
	{
		...
		//$cmid = $view_controller->check_peoplecompanyid($id);
		$user_details = $view_controller->details_userinfo($id);
		if($usertype == 'C')
		{
		...
		$view = View::factory('admin/managerdetails')
			    	->bind('pag_data',$pag_data)
				->bind('user_details',$user_details)
				->bind('id',$id)
            			->bind('Offset',$offset);
	...
	public function action_getcompanymanagerlist()
	{
		...
		$getmodel_details = $manage_model->get_companymanagerlist($company_id,$offset, REC_PER_PAGE);
			...
			if($listings['status']=='A'){  $txt = "Deactivate"; $class ="unsuspendicon";    }
	...
	//<div class="button greyish"><a href="'.URL_BASE.'manageusers/passengers">View All</a></div>
				...
				else {  $txt = $listings['rating'].' / 5'; }
				if(empty($listings['comments']))
				{  $txt1 = "No comments"; }

				else {  $txt1 = $listings['comments']; }
				$drop = "-";
	...
	public function action_getmanagerdriverlist()
	{
		...
		//echo '<pre>';print_r($getmodel_details);exit;
		$count=count($getmodel_details);
		$param_id = $getmodel_details[0]['cid'];
				...
				<div style="width:auto; float:right; margin:0;">';
				if($count > 5){ 
				$output .='<a class="export_me_menu" href="'.URL_BASE.'manage/driversearch?keyword=&status=&filter_company='.$param_id.'">View All</a>                       
				</div>';
				}
				else
				{
	...
	public function action_transaction()
	{
		...
		$view = View::factory('admin/transactiondetails')
				->bind('Offset',$offset)
				->bind('action',$action)
				->bind('srch',$_REQUEST)
				->bind('pag_data',$pag_data)
	...
	public function action_get_translist()
	{
		...
		$getmodel_details = $manage_model->get_translist($company_id,$offset, REC_PER_PAGE);
			...
			$count=count($getmodel_details);
				...
				else { $output .='<td>'.$listings['rating'].'</td>'; }
	...
	public function action_driver_unavailable()
	{
		...
		if ($signup_submit && Validation::factory($_POST) ) 
		{
			...
			$validator = $view_controller->validate_unavailabledriver(arr::extract($post,array('driver_id','reason','startdate','enddate')));
		...
		$view = View::factory('admin/driver_unavailable')
			    	->bind('pag_data',$pag_data)
	...
	public function action_unavailability()
	{
		...
		$view = View::factory('admin/manage_unavailability')
			    	->bind('pag_data',$pag_data)
				->bind('all_unavailablity_list',$all_unavailablity_list)
				->bind('id',$id)
            			->bind('Offset',$offset);
	...
	{	
		...
		//send data to view file 
		$view = View::factory('admin/manage_availabilitytaxi')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
	...
	public function action_availabilitydriver()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_availabilitydriver')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
	...
	public function action_menu()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_menu')
				->bind('all_menu_list',$all_menu_list)
				->bind('pag_data',$pag_data)
				->bind('srch',$_REQUEST)
				->bind('Offset',$offset);
	...
	public function action_mile()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_mile')
				->bind('all_mile_list',$all_mile_list)
				->bind('pag_data',$pag_data)
				->bind('MileList',$MileList)
				->bind('srch',$_REQUEST)
				->bind('Offset',$offset);
	...
	{	
		...
		//send data to view file 
		$view = View::factory('admin/manage_today_unassigned_taxi')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
	...
	public function action_freetaxi()
	{
		...
		//$all_company_list = $manage_company->all_assigntaxi_list($offset, REC_PER_PAGE);
		...
		//send data to view file 
		$view = View::factory('admin/manage_freetaxi')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
		...
		public function action_unassign_driver()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_unassign_driver')
				->bind('all_company_list',$all_company_list)
				->bind('get_allcompany',$get_allcompany)
			    	->bind('pag_data',$pag_data)
				->bind('CompanyList',$CompanyList)
	...
	public function action_banner()
	{
		...
		$view = View::factory('admin/manage_banner')
				->bind('all_banner_list',$all_banner_list)
			    ->bind('pag_data',$pag_data)
				->bind('srch',$_REQUEST)
            ->bind('Offset',$offset);
	...
	public function action_faq()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_faq')
				->bind('all_faq_list',$all_faq_list)
			    ->bind('pag_data',$pag_data)
				->bind('srch',$_REQUEST)
            ->bind('Offset',$offset);
			...
			if(!empty($get_transaction))
			{
				...
				//exit;
				 $output='<div id="transaction_chart" style="min-width: 400px; height: 400px; margin: 0 auto;'.$display.'"></div>';
					...
					title: {
						text: 'Transaction from <?php echo Commonfunction::getDateTimeFormat($start_date,1); ?> to <?php echo Commonfunction::getDateTimeFormat($end_date,1); ?>',
						x: -20 //center
					},
					subtitle: {
						text: '',
	...
	public function action_taxi_transaction_search()
	{
		...
		if(isset($_POST))
		{
			...
			$driver_id=$_POST['driver_id'];
			$output="";
			$for_date = Commonfunction::getDateTimeFormat($start_date,1)." to ".Commonfunction::getDateTimeFormat($end_date,1);
		...
		$('#transaction_chart').highcharts({			
			...
			title: {
					text: 'Total Trip Details [<?php echo $taxi_name; ?>]'
				},
				subtitle: {
		...
		echo $output.'~'.$totTransCount;exit;
	}
	...
	public function action_promocode()
	{
		...
		//send data to view file 
		$view = View::factory('admin/manage_promocode')
				->bind('promocode_list',$promocode_list)
			    ->bind('pag_data',$pag_data)
	...
	public function action_promoinfo()
	{
		...
		$view = View::factory('admin/promoinfo')
				->bind('promo_data',$details);
	...
	public function action_driver_logout()
	{
		...
		$driver_model = Model::factory('driver');
		$customer_google_api = $Commonmodel->select_site_settings('driver_android_key',SITEINFO);
		$get_driver_log_details = $manage_company->get_driver_current_status($driver_id,$company_id);
		//echo count($get_driver_log_details);exit;
		if(count($get_driver_log_details) == 0)
		{
			//send push notification to driver
			$driverDeviceDets = $manage_company->getDriverDeviceToken($driver_id);
			if(count($driverDeviceDets) > 0 && !empty($driverDeviceDets[0]['device_token'])){
				$pushMessage = array("message" => __('driver_logout_via_admin'),"status"=>15,"display"=>1);
		  ...
		  $login_status_update = $Commonmodel->update(PEOPLE,$update_array,'id',$driver_id);
		  /** GET Shift ID **/
		...
		$transaction = $Commonmodel->update(DRIVERSHIFTSERVICE,$shiftupdate_arrary,'driver_shift_id',$driver_shift_id);
		}
	...
	public function action_walletrequests()
	{
		...
		if(isset($_SESSION['download_set'])) {
			...
			if($usertype == 'A') {
				$export_table_header= array(__('name'),__('phone_label'),__('companyname'),__('request_amount'),__('request_date'),__('request_status'));
				$export_table_field_select= array('driverName','driverPhone','company_name','wallet_request_amount','wallet_request_date','reqstatus');
			} else {
				$export_table_header= array(__('name'),__('phone_label'),__('request_amount'),__('request_date'),__('request_status'));
				$export_table_field_select= array('driverName','driverPhone','wallet_request_amount','wallet_request_date','reqstatus');
			}
			$heading='taxiexport';
			$data = array();
			if(count($allRequests) > 0){
				foreach($allRequests as $key => $val){
					...
					$allRequests[$key]['wallet_request_status'] = $wallet_request_status;
				}
			}
		...
		$taxicompany_details = $add_model->taxicompany_details();
		//send data to view file 
		$view = View::factory('admin/driver_wallet_request')
				->bind('requestLists',$getDriverWalletRequests)
				->bind('countDriverRequests',$countDriverRequests)
				->bind('taxicompany_details',$taxicompany_details)
				->bind('pag_data',$pag_data)
			    ->bind('srch',$_GET)
				->bind('Offset',$offset);
	...
	public 	function action_driverreferralhistory()
	{
		...
		$view = View::factory('admin/driver_referral_list')
				->bind('pag_data',$pag_data)
				->bind('driverReferralList',$get_driver_referral_list);
	...
	public function action_withdrawrequest()
	{
		...
		if(isset($_SESSION['download_set'])){
			if(count($all_data) > 0) {
				foreach($all_data as $key => $value) {
					if(array_key_exists('brand_type', $value)) {
						$brand_type = ($value['brand_type'] == 1) ? "Multy" : "Single";
					...
					if(array_key_exists('request_status', $value)) {
						...
						if($value['request_status'] == 1) {
							...
							$status_label = "Cancelled by driver";
						}
			...
			if($company_id == 0) {
				$export_table_header = array(__('request_id'),__('brand_type'),__('name'),__('withdraw_amount'),__('request_date'),__('request_status'));
				$export_table_field_select = array('request_id','brand_type','name','withdraw_amount','request_date','request_status');
			} else {
				$export_table_header = array(__('request_id'),__('withdraw_amount'),__('request_date'),__('request_status'));
				$export_table_field_select = array('request_id','withdraw_amount','request_date','request_status');
			}
		...
		//echo '<pre>';print_r($dashboard_withdraw_request);exit;
		$view = View::factory('admin/withdraw/admin_withdraw_request')
				->bind('dashboard_data',$dashboard_withdraw_request)
				->bind('data',$get_withdraw_request)
				->bind('count_data',$count_data)
				->bind('pag_data',$pag_data)
				->bind('srch',$_GET)
	...
	public function action_withdrawdeatil()
	{
		...
		$view = View::factory('admin/withdraw/withdraw_details')
	...
	public function action_driverwithdraw()
	{
				...
				foreach($all_data as $key => $value) {
					...
					if(array_key_exists('request_status', $value)) {
						...
						if($value['request_status'] == 1) {
							...
							$status_label = "Rejected (Cancelled by admin)";
						} else if($value['request_status'] == 3) {
							$status_label = "Rejected (Cancelled by driver)";
						}
						unset($value['request_status']);
						$all_data[$key]['request_status'] = $status_label;
					}
		...
		$view = View::factory('admin/withdraw/driver_withdraw_request')
				->bind('dashboard_data',$dashboard_withdraw_request)
				->bind('data',$get_withdraw_request)
				->bind('count_data',$count_data)
				->bind('pag_data',$pag_data)
		...
		$this->template->title = SITENAME." | ".__('request_list');
		$this->template->page_title = __('driver_withdraw'); 
		$this->template->content = $view;
	}


	public function action_pdftaxiinfo()
	{
		$id = $this->request->param('id');		
		$view_controller = Model::factory('manage');
		$view_model = Model::factory('driver');	
		...
		//echo count($driver_logs_completed_transaction);		
			...
			p#second > span {}
			table.first {color: #003300;font-family: helvetica;font-size: 8pt;background-color:#FFF; border:10px solid #236B8D;}
			...
			<tr>
			<td></td>
			<td><div class="invoice_head">'.date("F j, Y").'<h1>Taxi Completed Trips</h1></div></td>
			</tr>
			<tr>';

			$Tophtml .='<td align="left"></td>
			<td align="right">'.$taxi_details[0]['taxi_no'].'</td>';
			...
			$Middle_html .= '<table border="0" cellpadding="1" cellspacing="1" width="700">
						...
						($i%2 == 1)?$class="eventr":$class="oddtr";
							...
							foreach($taxi_logs_completed_transaction as $values)
							{
								...
								$Middle_html .= '<tr>	
								<td width=10>'.$i.'</td>
								<td width=130>'.$values->passengers_log_id.'</td>
								<td width=130>'.ucfirst($values->name).'</td>
								<td width=130>'.$values->current_location.'</td>
								<td width=350>'.$values->drop_location.'</td>
								<td width=350>'.Commonfunction::getDateTimeFormat($values->pickup_time,1).'</td>
								<td width=130>'.$distance.'</td>
								<td width=30>'.$company_currency.$current_fare.'</td>
								</tr>';
			...
			//exit;
			ob_clean();			
			$filename = $taxi_details[0]['taxi_no'].'-'.date('m-d-y-s');
			$generate_pdf = $manage->generate_pdf($html,$filename);
	...
	public function action_assign_mulit_date()
	{
		$this->is_login();
		$ids = $_GET['uniqueId'];
		...
		$manage = Model::factory('manage');
		$date= $manage->update_assign_date($ids,$start_date,$end_date);
		
		Message::success(__('Taxi Date has been assigned successfully.'));
 
		$this->request->redirect($_SERVER['HTTP_REFERER']);
	}
	
	//Action to manage driver coupons
	public function action_coupon()
	{
		...
		}else
		{
			...
			//Page Title
		$this->page_title =  __('manage_coupon');
		$this->selected_page_title = __('manage_coupon');
		...
		$couponcode_list_count = $manage->couponcode_list_count($search, $company_id);
		$couponcode_list = $manage->couponcode_list($offset, REC_PER_PAGE,$search, $company_id);
		$total_users = count($couponcode_list_count);
		$pag_data = Pagination::factory(array (
		      'current_page'   => array('source' => 'query_string','key' => 'page'),
			  'items_per_page' => REC_PER_PAGE,
		...
		//send data to view file 
		$view = View::factory('admin/manage_couponcode')
				->bind('couponcode_list',$couponcode_list)
			    ->bind('pag_data',$pag_data)
			    ->bind('total_users',$total_users )
			    ->bind('srch',$_REQUEST)
				->bind('Offset',$offset);
		...
		$this->template->title= SITENAME." | ".__('manage_coupon');
		$this->template->page_title= __('manage_coupon'); 
		$this->template->content = $view;				
	}
	
	
	//Action to manage driver coupons
	public function action_generated_coupon_list()
	{
		...
		if(isset($_GET['search_user']))
		{
			//$search = array_map("mysql_real_escape_string", $_GET);
			$search=$_GET;
		}else
		{
			$search = ''; 
			...
			//Page Title
		$this->page_title =  __('generated_coupon_list');
		$this->selected_page_title = __('generated_coupon_list');
		...
		$generate_id_list_count = $manage_model->coupon_generate_id_list_count($search);
		$generate_id_list = $manage_model->get_coupon_generate_id_list($offset, REC_PER_PAGE,$search);
		// print_r($generate_id_list);
		// exit;
		$total_records = $generate_id_list_count;
		$pag_data = Pagination::factory(array (
		      'current_page'   => array('source' => 'query_string','key' => 'page'),
			  'items_per_page' => REC_PER_PAGE,
		...
		//send data to view file 
		$view = View::factory('admin/manage_generate_coupon_list')
				->bind('generate_id_list',$generate_id_list)
			    ->bind('pag_data',$pag_data)
			    ->bind('total_records',$total_records )
			    ->bind('srch',$_REQUEST)
				->bind('Offset',$offset);


		$this->template->title= SITENAME." | ".__('generated_coupon_list');
		$this->template->page_title= __('generated_coupon_list'); 
		$this->template->content = $view;				
	}
	
	public function action_reprint_coupon()
	{
		$generate_id = $_REQUEST['generate_id'];

		$manage_model = Model::factory('manage');

		$driver_coupons_list = $manage_model->get_generate_id_coupons($generate_id);

		Session::instance()->set('drivers_coupons', $driver_coupons_list);

		Message::success( __( 'Driver coupon regenerated successfully' ) );
	...
	public function action_passenger_coupon()
	{
		...
		$couponcode_list_count = $manage->passenger_couponcode_list_count($search, $company_id);
		$couponcode_list = $manage->passenger_couponcode_list($offset, REC_PER_PAGE,$search, $company_id);
		$total_users = count($couponcode_list_count);
		$pag_data = Pagination::factory(array (
		      'current_page'   => array('source' => 'query_string','key' => 'page'),
			  'items_per_page' => REC_PER_PAGE,
		...
		//send data to view file 
		$view = View::factory('admin/manage_passenger_couponcode')
				->bind('couponcode_list',$couponcode_list)
			    ->bind('pag_data',$pag_data)
			    ->bind('total_users',$total_users )
			    ->bind('srch',$_REQUEST)
	...
	//Action to manage driver coupons
	public function action_passenger_generated_coupon_list()
	{
		$this->is_secondary_login();
		$user_createdby = $_SESSION['userid'];
		$company_id = $_SESSION['company_id'];
		...
		$generate_id_list_count = $manage_model->coupon_generate_id_list_count_passenger($search);
		$generate_id_list = $manage_model->get_coupon_generate_id_list_passenger($offset, REC_PER_PAGE,$search);
		// print_r($generate_id_list);
		// exit;
		$total_records = $generate_id_list_count;
		$pag_data = Pagination::factory(array (
		...
		//send data to view file 
		$view = View::factory('admin/manage_passenger_generate_coupon_list')
				->bind('generate_id_list',$generate_id_list)
			    ->bind('pag_data',$pag_data)
			    ->bind('total_records',$total_records )
			    ->bind('srch',$_REQUEST)
	...
	public function action_passenger_reprint_coupon()
	{
		$this->is_secondary_login();
		$generate_id = $_REQUEST['generate_id'];
		...
		$driver_coupons_list = $manage_model->get_generate_id_coupons_passengers($generate_id);

		Session::instance()->set('drivers_coupons', $driver_coupons_list);

		Message::success( __( 'Passenger coupon regenerated successfully' ) );
		//$this->request->redirect('print/coupon');
		$data = array();
		$data['status'] = 1;
		$data['redirect_url'] = URL_BASE.'print/coupon';
	...
	public function action_trip_balance_report()
	{
		$this->is_secondary_login();
		$company_id = $_SESSION['company_id'];
		$usertype = $_SESSION['user_type'];
		...
		$dashboard_data = $manage->get_dashboard_data($_GET,$company_id);

		
		if(isset($_GET['trip_search_user']))
		{
			//$search = array_map("mysql_real_escape_string", $_GET);
			$search=$_GET;
		}else
		...
		$count_data = $manage->count_admintransaction_list($search,$company_id);
		...
		$all_transaction_list = $manage->admintransaction_list($search,$offset, REC_PER_PAGE,$company_id);
//print_r($dashboard_withdraw_request); exit;

		$view = View::factory('admin/withdraw/trip_balance_report')
				->bind('dashboard_data',$dashboard_data)
				->bind('data',$all_transaction_list)
				->bind('count_data',$count_data)
				->bind('pag_data',$pag_data)
				->bind('srch',$_GET)
				->bind('company_id',$company_id)
		...
		$this->template->title= SITENAME." | ".__('trip_balance_report');
		$this->template->page_title = __('trip_balance_report'); 
		$this->template->content = $view;
	}
	
	/** approve the driver wallet requests **/
	public function action_approveWalletRequest_trip()
	{
		$manage = Model::factory('manage');
		$result = $manage->updateRequestStatus_trip($_REQUEST['uniqueId'],'1',$this->crntTime,$this->session->get('userid'));
		//Flash message for Approve
		//==========================
		Message::success(__('driver_wallet_request_approved'));
		//redirects to job_feedback details page after deletion
	...
	/** disapprove the driver wallet requests **/
	public function action_disapproveWalletRequest_trip()
	{
		$manage = Model::factory('manage');
		$result = $manage->updateRequestStatus_trip($_REQUEST['uniqueId'],'2',$this->crntTime,$this->session->get('userid'));
		//Flash message for Approve
		//==========================
		Message::success(__('driver_wallet_request_disapproved'));
		//redirects to job_feedback details page after deletion
