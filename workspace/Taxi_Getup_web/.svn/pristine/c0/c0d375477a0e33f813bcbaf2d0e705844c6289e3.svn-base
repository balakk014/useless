...
<?php /** Dashboard total details count Start * */ ?>
	...
	<div class="dashboard_widget_row">
		<div class="dashborad_widget_box lg-12">
			<div class="dashboard_widget_title with_calender">
				...
				<div class="dashboard_calender_in_header with_select_box trip_req_from_city">
					<ul>
						...
						<li>
							<input type="button" name="search_assign_unassign" onclick="assignUnassignedChart();" value="GO" title="Go" >
						</li>
					</ul>
				</div>
				<?php } else { ?>
			...
			<div class="dashboard_map_outer">
				<div class="assignedUnassignedChartAjax"></div>
			</div>
		</div>
    </div>
	...
	<div class="dashborad_widget_box lg-6">
	    ...
	    <div class="dashboard_widget_title with_calender">
    		...
    		<div class="dashboard_calender_in_header with_select_box">
    			...
    			<li>
    			    <input type="button" name="search_company_wise_trip" onclick="companyWiseTripChart();" value="GO" title="Go" >
    			</li>
    		    </ul>
    		</div>    		
    	    </div>
    	    <div class="dashboard_map_outer">
    		<div class="companyWiseTripChartAjax"></div>
    	    </div>
	...
	<div class="dashborad_widget_box <?php if ($_SESSION['company_id'] == 0) { ?> lg-6 <?php } else { ?> lg-12 <?php } ?>">
		...
		<div class="dashboard_calender_in_header with_select_box payment_box payment_by_company" style="margin-top:11px;">
			...
			<li>
			    <input type="button" name="search_payment_by_company" onclick="paymentByCompanyChart();" value="GO" title="Go" >
			</li>
		    </ul>
		</div>
		</div>
	    <div class="dashboard_map_outer">
		<div class="paymentByCompanyChartAjax <?php if ($_SESSION['company_id'] > 0) { ?>payment_by_company<?php } ?>"></div>
	...
	<div class="dashborad_widget_box lg-12">
		...
		<div class="dashboard_calender_in_header with_select_box trip_req_from_city">
		    ...
		    <span id="city_by_count_enddate_error" class="errors" style="display:none;"></span>
			    ...
			    <li>
				<input type="button" name="search_city_by_count" onclick="cityByCountChart();" value="GO" title="Go" >
			    </li>
		    </ul>
		</div>
	    ...
	    <div class="dashboard_map_outer">
		<div class="cityByCountChartAjax"></div>
	    </div>
	</div>	    
    </div>
...
<script type="text/javascript" language="javascript">
	...
	function assignUnassignedChart()
	{
		...
		$('.assignedUnassignedChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');
	...
	if(startdate !='' && enddate!='') {
	    $('.companyWiseTripChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
	    if(to_timestamp(startdate) > to_timestamp(enddate)) {
	...
	if(startdate !='' && enddate!='') {
	    $('.paymentByCompanyChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
	    if(to_timestamp(startdate) > to_timestamp(enddate)) {
    ...
    /* function tripReqTopCitiesChart()
{
	...
	if(startdate !='' && enddate!='') {
	    $('.cityByCountChartAjax').html('<img alt="ajax-loading" src="'+SrcPath+'/public/css/img/ajax-loaders/ajax-loader-1.gif" />');	
	    if(to_timestamp(startdate) > to_timestamp(enddate)) {
