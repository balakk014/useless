...
class Controller_TaximobilityManager extends Controller_Siteadmin
{
	...
	{	
		...
		{ 
			...
			$this->template->page_title= __('manager_login_title');
	    		...
	    		$view=View::factory(MANAGERVIEW.'login')
	...
	public function action_dashboard()
	{
		...
		{	
			...
			{
				$gettransaction = $dashboard->changegettransaction($driver_id,Commonfunction::ensureDatabaseFormat($_REQUEST['userstartdate'],1),Commonfunction::ensureDatabaseFormat($_REQUEST['userenddate'],2));
			}
			else
			{
				$gettransaction = $dashboard->gettransaction($driver_id);					
		...
		$view= View::factory('manager/dashboard')
			...
			->bind('name', $name)
			->bind('gettransaction', $gettransaction)
			->bind('count', $count)
	...
	*/
	...
	public function action_forgot_password()
	{
		...
		if(isset($forgotpassword) && Validation::factory($_POST) )
		{
			...
			$validator = $this->authorize->forgotpassword_managervalidate(arr::extract($post,array('email')));
		...
		$view=View::factory(MANAGERVIEW.'forgot_password')
				->bind('errors',$errors)
				->bind('postvalue',$postvalue);
