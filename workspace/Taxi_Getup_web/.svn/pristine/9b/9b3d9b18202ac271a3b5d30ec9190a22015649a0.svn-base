...
class Model_Finds extends Model
{
...
'bookingtype','bookby');	
	...
	public function nearestdrivers($lat,$long,$taxi_model,$passenger_id,$distance = NULL,$company_id,$unit,$service_type)
	{
		...
		if($unit == '0')
		{
			...
			$query =" select list.name as name,list.driver_id as driver_id,list.latitude as latitude,list.longitude as longitude,list.d_distance as distance_km,taxi.taxi_id as taxi_id,taxi.taxi_capacity,taxi.taxi_speed as taxi_speed,list.updatetime_difference as updatetime_difference from ( SELECT people.name,driver.*,(((acos(sin((".$lat."*pi()/180)) * sin((driver.latitude*pi()/180))+cos((".$lat."*pi()/180)) *  cos((driver.latitude*pi()/180)) * cos(((".$long."- driver.longitude)* pi()/180))))*180/pi())*60*1.1515$unit_conversion) AS d_distance,(TIME_TO_SEC(TIMEDIFF('$current_time',driver.update_date))) AS updatetime_difference 
