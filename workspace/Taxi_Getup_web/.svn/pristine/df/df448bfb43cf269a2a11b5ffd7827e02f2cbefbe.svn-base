    ...
    <div class="cont_container mt15 mt10">
       <div class="content_middle">
           <div class="driverinfo_common">
			...
			<li><label><?php echo __('rating_points'); ?></label>
				   <p><?php if(count($get_tot_ratings_driver) > 0) { 
					   $rate = ($get_tot_ratings_driver[0]['total_ratings'] + 5)/($get_tot_ratings_driver[0]['trip_cnt']+1); 
			...
			<?php if($usertype == 'A') { ?>
			   ...
			   <li><label><?php echo __('companyaddress'); ?></label>
                               <p><?php if(isset($user_details1[0]['company_address']) && !empty($user_details1[0]['company_address'])) { echo $user_details1[0]['company_address']; }else{ echo $user_details1[0]['address']; } ?></p>				
				   </li>  	          
		...
		<div class="widget margin-bottom" >
			<div class="title"><h6><?php echo __('Current Status of Driver'); ?></h6>
				<?php if(($_SESSION["user_type"]=='C' || $_SESSION["user_type"]=='M' || $_SESSION["user_type"]=='A') && $user_details1[0]['login_status']=='S') {  ?>
                                    <div class="small_butt">
					<input type="button" name="logout" id="driver_logout"  onclick="d_logout('<?php echo $user_details1[0]['id']; ?>','<?php echo $user_details1[0]['company_id']; ?>')" value="logout" title="logout" >
					</div>
                                <?php } ?>
...
?>			
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      map = new GMaps({
        el: '#map',
        lat: <?php echo isset($driver_centerlat)?$driver_centerlat:0; ?>,
        lng: <?php echo isset($driver_centerlng)?$driver_centerlng:0; ?>,
        click: function(e){
          //console.log(e);
        }
      });
var iconBase = '<?php echo PUBLIC_IMGPATH.'/' ; ?>';
   /** Make Marker for three paths **/
	...
	<?php if(!empty($driver_endlat) && !empty($driver_endlng)) { ?>
      map.addMarker({
        lat: <?php echo isset($driver_endlat)?$driver_endlat:0; ?>,
        lng: <?php echo isset($driver_endlng)?$driver_endlng:0; ?>,
	icon: iconBase + 'driver_one.png',
	...
	map.addMarker({
        lat: <?php echo isset($driver_centerlat1)?$driver_centerlat1:0; ?>,
        lng: <?php echo isset($driver_centerlng1)?$driver_centerlng1:0; ?>,
	icon: iconBase + 'driver_two.png',
      ...
      map.addMarker({
        lat: <?php echo isset($driver_endlat1)?$driver_endlat1:0; ?>,
        lng: <?php echo isset($driver_endlng1)?$driver_endlng1:0; ?>,
	icon: iconBase + 'driver_two.png',
        title: '<?php echo $driver_tracking[1]['drop_location']; ?>',
      });
	<?php } ?>
		path1 = <?php echo isset($driver_latlog1)?$driver_latlog1:0; ?>;
	...
	map.addMarker({
        lat: <?php echo isset($driver_centerlat2)?$driver_centerlat2:0; ?>,
        lng: <?php echo isset($driver_centerlng2)?$driver_centerlng2:0; ?>,
	...
	icon: iconBase + 'driver_three.png',
      ...
      map.addMarker({
        lat: <?php echo isset($driver_endlat2)?$driver_endlat2:0; ?>,
        lng: <?php echo isset($driver_endlng2)?$driver_endlng2:0; ?>,
	icon: iconBase + 'driver_three.png',
	...
	path2 = <?php echo isset($driver_latlog2)?$driver_latlog2:0; ?>;
...
<?php } ?>
	...
	<!-- Last Three Trips Driver Map ends -->
			...
			<?php if($total_trip_statitics > 0) { ?>
			 <form  action="#" method="post" name="" id="" >
				...
				?>
				 ...
				 <input type="hidden" name="user_name" id="user_name" value="<?php echo $driver_name; ?>">
				  <input type="hidden" name="user_type" id="user_type" value="D">
				 <input type="text"  readonly title="<?php echo __('select_datetime'); ?>"  id="tripstartdate" name="transstartdate" value="<?php echo Commonfunction::getDateTimeFormat($trip_startdate,1); ?>"  />
				 <span id="tripstartdate_error" class="errors" style="display:none;"></span>
                                 </div>
                                 <div class="small_field_det">
                                     <label><?php echo __('enddate');?></label>
				 <input type="text"  readonly title="<?php echo __('select_datetime'); ?>"  id="tripsenddate" name="transenddate" value="<?php echo Commonfunction::getDateTimeFormat($trip_enddate,1); ?>"  />
		...
		<!-- Ongoing Journey -->
		<div class="widget margin-bottom" >
			<div class="title"><h6><?php echo __('Ongoing Journey'); ?></h6>
				<div style="width:auto; float:right; margin: 0px;">		
				<div class="button greyish"></div>
			...
			<div>
					...
					<table cellspacing="1" cellpadding="10" width="100%" align="center" class="sTable responsive">
						...
						<tr>
							...
							<td><b><?php echo __('cancel_trip_label');?></b></td>
						...
						($i%2 == 1)?$class="eventr":$class="oddtr";
							...
							foreach($driver_logs_progress as $values)
							{
								...
								<tr class="<?php echo $class; ?>">	
									...
									<td><?php echo date('h:i:s', strtotime($values->pickup_time));?></td>
									<td>
									<?php if(isset($values->driver_reply) && $values->travel_status =='9' && $values->driver_reply =='A' || 
											$values->travel_status =='2' && $values->driver_reply =='A' ||
											$values->travel_status =='3' && $values->driver_reply =='A' ||
											$values->travel_status =='5' && $values->driver_reply =='A') {?>	
									<input type="text" title="<?php echo __('comments'); ?>" id="<?php echo 'trip_'.$values->passengers_log_id.'_comments'; ?>" name="ongoing_startdate" value="<?php echo $values->comments; ?>" placeholder="<?php echo __('enter_comments'); ?>" />
									<?php } else {?>
										-
									<?php } ?>	
									</td>
									<td style="text-align:center;" id="cancel_trip_td-<?php echo $values->passengers_log_id; ?>">
									<?php 
									if(isset($values->driver_reply) && $values->travel_status =='9' && $values->driver_reply =='A' || $values->travel_status =='3' && $values->driver_reply =='A')
									{
										?>
										<div class="button blackB">
										<input type="button" name="cancel_trip" id="cancel_trip"  onclick="cancel_trip('<?php echo $values->passengers_log_id; ?>')" value="Cancel Trip" title="Cancel Trip" >
										</div>
									<?php } else {?>
										-
									<?php } ?>	
									</td>
									<td style="text-align:center;" id="complete_trip_td-<?php echo $values->passengers_log_id; ?>">
									<?php 
									if($values->travel_status =='2' || $values->travel_status =='5')
									{
										?>
										<div class="button blackB">
										<input type="button" name="complete_trip" id="complete_trip"  onclick="complete_trip('<?php echo $values->passengers_log_id; ?>')" value="Complete Trip" title="Complete Trip" >
										</div>
									<?php } else {?>
										-
									<?php } ?>	
		...
		<div class="widget margin-bottom comp_journy" >
			...
			<?php if($get_tot_trans_driver > 0) { ?>
			 ...
			 <div class="rgt_field_det" align="right">
				...
				?>
				 ...
				 <input type="hidden" name="user_name" id="user_name" value="<?php echo $driver_name; ?>">
				  <input type="hidden" name="user_type" id="user_type" value="D">
				 <input type="text"  readonly title="<?php echo __('select_datetime'); ?>" id="userstartdate" name="userstartdate" value="<?php echo Commonfunction::getDateTimeFormat($users_startdate,1); ?>"  />
				 <span id="startdate_error" class="errors" style="display:none;"></span>
                                </div>
                                <div class="small_field_det">
                                 <label><?php echo __('enddate'); ?></label>
				 <input type="text"  readonly title="<?php echo __('select_datetime'); ?>" id="userenddate" name="userenddate" value="<?php echo Commonfunction::getDateTimeFormat($users_enddate,1); ?>" />
			...
			<div id="drivercompleted_logs">
				...
				<div align="left" class="small_butt"> 
		...
		*/ ?>
					...
					<table cellspacing="1" cellpadding="10" width="100%" align="center" class="sTable responsive">
						...
						($i%2 == 1)?$class="eventr":$class="oddtr";
							...
							{ ?>
								...
								<td><?php echo Commonfunction::getDateTimeFormat($values->shift_start,1); ?></td>
								<td><?php echo Commonfunction::getDateTimeFormat($values->shift_end,1); ?></td>
								<td>
		...
		<!-- Service Time Journey -->
			...
			if(count($user_details)>0){
			 ...
			 foreach($user_details as $res){
				...
				if($res->profile_image && file_exists($_SERVER['DOCUMENT_ROOT'].'/public/uploads/passenger/thumb_'.$res->profile_image))
				{
		...
		<div class="widget margin-bottom" >
			<div class="title">
				...
				<div style="width:auto; float:right; margin:0;">
					<?php if(count($driverReferralList) > REC_PER_PAGE) { ?>
...
<input type="hidden" name="driver_id" id="driver_id" value="<?php echo $user_details1[0]['id']; ?>">
...
<?php } ?>
<script>
	function complete_trip(trip_id, cash_payment)
	{
		var comments = $('#trip_'+trip_id+'_comments').val();
		...
		var r = false;
		if(cash_payment == 0)
		var r = confirm("Do you want complete the selected trip ?");

		if ((r == true && cash_payment == 0) || cash_payment == 1) {
		var dataS = "trip_id="+trip_id+"&cash_payment="+cash_payment+"&comments="+comments;
		var old_completetrip_html = $("#complete_trip_td-"+trip_id).html();
		$("#complete_trip_td-"+trip_id).html("<img alt='ajax-loading' src='<?php echo URL_BASE; ?>public/css/img/ajax-loaders/ajax-loader-1.gif' />");
		$.ajax
		({
			...
			success: function(data)
			{
				if(data == 1)
				{
				 ...
				 $('#complete_trip_td-'+trip_id).html('<div class="button blackB"><input type="button" name="complete_trip"   value="Trip Completed" title="Trip Completed" ></div>');
				}
				else if(data == 2)
				{
				...
				{
					$("#complete_trip_td-"+trip_id).html(old_completetrip_html);
	...
	function cancel_trip(trip_id)
	{
		...
		var r = false;
		var r = confirm("Do you want cancel the selected trip ?");

		if (r == true) {
		var dataS = "trip_id="+trip_id+"&comments="+comments;
		var old_canceltrip_html = $("#cancel_trip_td-"+trip_id).html();
		$("#cancel_trip_td-"+trip_id).html("<img alt='ajax-loading' src='<?php echo URL_BASE; ?>public/css/img/ajax-loaders/ajax-loader-1.gif' />");
		$.ajax
		({
		 
			type: "POST",
			url: SrcPath+"driver/driver_cancel_trip",
			data: dataS,
			cache: false,
			dataType: 'html',
			success: function(data)
			{
				if(data == 1)
				{
				 ...
				 $('#cancel_trip_td-'+trip_id).html('<div class="button blackB"><input type="button" name="cancel_trip"   value="Trip Cancelled" title="Trip Cancelled" ></div>');
				}
				else if(data = 2)
				{
					$("#cancel_trip_td-"+trip_id).html(old_canceltrip_html);
					alert('Sorry, The selected trip has already started. You could not cancel in progress trip.')
				}	
				else
				{
					$("#cancel_trip_td-"+trip_id).html(old_canceltrip_html);
					alert('Error while cancelling the selected trip');
				}
			},
			error: function()
			{
